...
authors:
  - Team Nimiq
image: /assets/images/media/ZmqmMZm069VX1tCE_PaywithNIMBitcoinLightning3.3blue.png
---
...
2. The QR code is scanned and the app presents the equivalent NIM amount required to the user.
3. Those NIM are then locked into a smart contract and can be retrieved with secret X.
4. The app simultaneously sends the merchant's invoice to Nimiq’s Multi-Asset Lightning Node (MALN).
5. MALN fulfills the invoice using LN and by doing so learns secret X.
6. Using secret X, MALN reimburses itself by retrieving the NIM from the smart contract.
7. X is published on the NIM blockchain from which the app learns X.
8. The user can prove to the merchant they paid by knowing X.
...
![Lightning Network payment flow](/assets/images/media/Zmqcx5m069VX1s9W_Group48096084-4-.png)
...
![NIM to Lightning swap process](/assets/images/media/Zmqc05m069VX1s9X_Group48096085-4-.png)
...
![Lightning-enabled merchant checkout](/assets/images/media/ZmqlUZm069VX1tBn_image-15-.png)
