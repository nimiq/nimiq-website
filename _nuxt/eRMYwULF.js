import { n as H, t as q, i as G, d as le, e as B, f as we, p as J, g as $, h as ae, j as ge, k as be, m as ye, l as Se, o as Ee, q as Oe, r as Te, s as ue, v as Ce, b as xe, x as te } from "./DYDUvkDP.js";
import { j as k, O as E, X as z, E as _e, a0 as ce, B as Q, aB as T, U as Re, aU as Me, u as ke, d as ne, as as oe, aV as de, aW as We, a9 as Ae, a1 as De } from "./gdLxT4Lf.js";
function Ke(e = {}) {
...
const W = G ? window : void 0, Ne = G ? window.document : void 0;
function I(e) {
  var t;
  const n = T(e);
...
function F(...e) {
  const t = [], n = () => {
    ...
    var r, u;
    return [(u = (r = v.value) == null ? void 0 : r.map((i) => I(i))) != null ? u : [W].filter((i) => i != null), $(T(v.value ? e[1] : e[0])), $(ke(v.value ? e[2] : e[1])), T(v.value ? e[3] : e[2])];
  }, ([r, u, i, d]) => {
...
function fe(e, t, n = {}) {
  const { window: c = W, ...v } = n;
  let m;
  const a = X(() => c && "MutationObserver" in c), r = () => {
...
function Ye(e, t = {}) {
  const { immediate: n = true, fpsLimit: c = void 0, window: v = W, once: m = false } = t, a = E(false), r = k(() => c ? 1e3 / T(c) : null);
  let u = 0, i = null;
  function d(s) {
    if (!a.value || !v) return;
    u || (u = s);
...
function j(e, t = {}) {
  const { window: n = W, ssrWidth: c = ve() } = t, v = X(() => n && "matchMedia" in n && typeof n.matchMedia == "function"), m = E(typeof c == "number"), a = E(), r = E(false), u = (i) => {
    r.value = i.matches;
  };
  return Re(() => {
    if (m.value) {
  ...
  }), F(a, "change", u, { passive: true }), k(() => r.value);
}
...
function Ze(e, t = {}) {
  ...
  const { window: c = W, strategy: v = "min-width", ssrWidth: m = ve() } = t, a = typeof m == "number", r = a ? E(false) : { value: true };
  a && B(() => r.value = !!c);
  function u(s, l) {
...
const U = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, K = "__vueuse_ssr_handlers__", ze = Fe();
function Fe() {
...
function he(e, t, n, c = {}) {
  ...
  if (!n) return w;
  const b = T(t), O = je(b), N = (v = c.serializer) != null ? v : Be[O], { pause: _, resume: V } = we(w, () => D(w.value), { flush: m, deep: a, eventFilter: f });
  z(S, () => g(), { flush: m });
  let L = false;
  const p = (h) => {
    l && !L || g(h);
  ...
  o && r && (n instanceof Storage ? F(o, "storage", p, { passive: true }) : F(o, re, R)), l ? B(() => {
    L = true, g();
  }) : g();
  function x(h, C) {
    if (o) {
  ...
  function g(h) {
    if (!(h && h.storageArea !== n)) {
      ...
      if (!(h && h.key !== S.value)) {
        _();
        try {
          (h == null ? void 0 : h.newValue) !== N.write(w.value) && (w.value = A(h));
        } catch (C) {
          s(C);
        } finally {
          h ? De(V) : V();
...
const He = "*,*::before,*::after{-webkit-transition:none!important;-moz-transition:none!important;-o-transition:none!important;-ms-transition:none!important;transition:none!important}";
function $e(e = {}) {
  const { selector: t = "html", attribute: n = "class", initialValue: c = "auto", window: v = W, storage: m, storageKey: a = "vueuse-color-scheme", listenToStorageChanges: r = true, storageRef: u, emitAuto: i, disableTransition: d = true } = e, o = { auto: "", light: "light", dark: "dark", ...e.modes || {} }, f = Pe({ window: v }), s = k(() => f.value ? "dark" : "light"), l = u || (a == null ? le(c) : he(a, c, m, { window: v, listenToStorageChanges: r })), w = k(() => l.value === "auto" ? s.value : l.value), S = me("updateHTMLAttrs", (_, V, L) => {
    ...
    let A;
    d && (A = v.document.createElement("style"), A.appendChild(document.createTextNode(He)), v.document.head.appendChild(A));
    for (const g of R) p.classList.add(g);
    for (const g of x) p.classList.remove(g);
    D && p.setAttribute(D.key, D.value), d && (v.getComputedStyle(A).opacity, document.head.removeChild(A));
  });
  function b(_) {
    var V;
  ...
  z(w, O, { flush: "post", immediate: true }), B(() => O(w.value));
  const N = k({ get() {
...
function et(e, t, n = {}) {
  const { window: c = W, initialValue: v, observe: m = false } = n, a = E(v), r = k(() => {
    var i;
  ...
  return m && fe(r, u, { attributeFilter: ["style", "class"], window: c }), z([r, () => T(e)], (i, d) => {
    d[0] && d[1] && d[0].style.removeProperty(d[1]), u();
  }, { immediate: true }), z([a, r], ([i, d]) => {
    const o = T(e);
...
function nt(e = {}) {
  const { window: t = W } = e, n = E(1), c = j(() => `(resolution: ${n.value}dppx)`, e);
  let v = H;
...
function pe(e, t, n = {}) {
  const { window: c = W, ...v } = n;
  let m;
  const a = X(() => c && "ResizeObserver" in c), r = () => {
    m && (m.disconnect(), m = void 0);
  }, u = k(() => {
    const o = T(e);
...
function rt(e, t = {}) {
  ...
  return pe(e, S), z(() => I(e), (b) => !b && S()), fe(e, S, { attributeFilter: ["style", "class"] }), v && F("scroll", S, { capture: true, passive: true }), c && F("resize", S, { passive: true }), B(() => {
    m && S();
  }), { height: r, bottom: u, left: i, right: d, top: o, width: f, x: s, y: l, update: S };
}
function st(e, t = { width: 0, height: 0 }, n = {}) {
  const { window: c = W, box: v = "content-box" } = n, m = k(() => {
    var o, f;
...
function qe(e, t, n = {}) {
  const { root: c, rootMargin: v = "0px", threshold: m = 0, window: a = W, immediate: r = true } = n, u = X(() => a && "IntersectionObserver" in a), i = k(() => {
    const l = T(e);
    return $(l).map(I).filter(ue);
  });
  let d = H;
  const o = E(r), f = u.value ? z(() => [i.value, I(c), o.value], ([l, w]) => {
...
function it(e, t = {}) {
  const { window: n = W, scrollTarget: c, threshold: v = 0, rootMargin: m, once: a = false } = t, r = E(false), { stop: u } = qe(e, (i) => {
    let d = r.value, o = 0;
...
const se = 1;
function Ge(e, t = {}) {
  const { throttle: n = 0, idle: c = 200, onStop: v = H, onScroll: m = H, offset: a = { left: 0, right: 0, top: 0, bottom: 0 }, eventListenerOptions: r = { capture: false, passive: true }, behavior: u = "auto", window: i = W, onError: d = (p) => {
    console.error(p);
  ...
  function w(p, R) {
    ...
    const y = T(e);
    if (!y) return;
    (A = y instanceof Document ? i.document.body : y) == null || A.scrollTo({ top: (x = T(R)) != null ? x : l.value, left: (D = T(p)) != null ? D : s.value, behavior: T(u) });
  ...
  const S = E(false), b = oe({ left: true, right: false, top: true, bottom: false }), O = oe({ left: false, right: false, top: false, bottom: false }), N = (p) => {
    ...
    var R;
    if (!i) return;
    const x = ((R = p == null ? void 0 : p.document) == null ? void 0 : R.documentElement) || (p == null ? void 0 : p.documentElement) || I(p), { display: D, flexDirection: A, direction: g } = getComputedStyle(x), y = g === "rtl" ? -1 : 1, h = x.scrollLeft;
    O.left = h < o.value, O.right = h > o.value;
    const C = Math.abs(h * y) <= (a.left || 0), M = Math.abs(h * y) + x.clientWidth >= x.scrollWidth - (a.right || 0) - se;
    D === "flex" && A === "row-reverse" ? (b.left = M, b.right = C) : (b.left = C, b.right = M), o.value = h;
    let P = x.scrollTop;
    p === i.document && !P && (P = i.document.body.scrollTop), O.top = P < f.value, O.bottom = P > f.value;
    const Z = Math.abs(P) <= (a.top || 0), ee = Math.abs(P) + x.clientHeight >= x.scrollHeight - (a.bottom || 0) - se;
    D === "flex" && A === "column-reverse" ? (b.top = ee, b.bottom = Z) : (b.top = Z, b.bottom = ee), f.value = P;
  }, L = (p) => {
    var R;
    if (!i) return;
  ...
  return F(e, "scroll", n ? Ee(L, n, true, false) : L, r), B(() => {
    try {
      const p = T(e);
      if (!p) return;
      V(p);
  ...
  }), F(e, "scrollend", N, r), { x: s, y: l, isScrolling: S, arrivedState: b, directions: O, measure() {
    const p = T(e);
    i && p && V(p);
  } };
}
...
function at(e, t = {}) {
  const { onConnected: n, onDisconnected: c, onError: v, onMessage: m, immediate: a = true, autoConnect: r = true, autoClose: u = true, protocols: i = [] } = t, d = Q(null), o = E("CLOSED"), f = Q(), s = le(e);
  ...
  const V = () => {
    ...
    f.value = g, o.value = "CONNECTING", g.onopen = () => {
      ...
      if (o.value = "CLOSED", p(), l == null ? void 0 : l(), c == null ? void 0 : c(g, y), !S && t.autoReconnect && (f.value == null || g === f.value)) {
        const { retries: h = -1, delay: C = 1e3, onFailed: M } = Y(t.autoReconnect);
      ...
      if (t.heartbeat) {
        p();
        const { message: h = ie, responseMessage: C = h } = Y(t.heartbeat);
  ...
  if (t.heartbeat) {
    const { message: g = ie, interval: y = 1e3, pongTimeout: h = 1e3 } = Y(t.heartbeat), { pause: C, resume: M } = xe(() => {
  ...
  u && (G && F("beforeunload", () => R(), { passive: true }), q(R));
  const A = () => {
    !G && !te || (R(), S = false, b = 0, D());
  };
  return a && A(), r && z(s, A), { data: d, status: o, close: R, send: x, open: A, ws: f };
}
function ut(e = {}) {
...
function ct(e = {}) {
  const { window: t = W, initialWidth: n = Number.POSITIVE_INFINITY, initialHeight: c = Number.POSITIVE_INFINITY, listenOrientation: v = true, includeScrollbar: m = true, type: a = "inner" } = e, r = E(n), u = E(c), i = () => {
  ...
  if (F("resize", i, d), t && a === "visual" && t.visualViewport && F(t.visualViewport, "resize", i, d), v) {
