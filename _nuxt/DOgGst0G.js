import { t as j, n as ce, d as B, m as pe, e as we, i as G, f as be, g as $, h as ge, j as fe, k as q, l as ye, p as J, o as Se, q as Ee, s as Oe, v as xe, x as _e, a as Te, y as re } from "./B5vKjtsS.js";
import { a as N, aW as E, a1 as y, Q as P, d as se, r as Z, n as Re, ak as de, h as Me, a7 as We, aX as ie, aY as ve, aZ as Ce, a_ as Ie, av as Ne, q as Ve } from "./B8biU-UE.js";
function Ue(e = {}) {
...
function A(...e) {
  const t = [], n = () => {
    ...
    var s, a;
    return [(a = (s = v.value) == null ? void 0 : s.map((i) => I(i))) != null ? a : [W].filter((i) => i != null), j(E(v.value ? e[1] : e[0])), j(Me(v.value ? e[2] : e[1])), E(v.value ? e[3] : e[2])];
  }, ([s, a, i, u]) => {
...
function ee(e, t, n = {}) {
  ...
  let m;
  const c = Q(() => d && "MutationObserver" in d), s = () => {
...
function me() {
  const e = Ce() ? Ee(Pe, null) : null;
  return typeof e == "number" ? e : void 0;
}
function H(e, t = {}) {
  const { window: n = W, ssrWidth: d = me() } = t, v = Q(() => n && "matchMedia" in n && typeof n.matchMedia == "function"), m = y(typeof d == "number"), c = y(), s = y(false), a = (i) => {
    s.value = i.matches;
  };
  return We(() => {
    if (m.value) {
...
function Ke(e, t = {}) {
  function n(o, l) {
    let w = E(e[E(o)]);
    return l != null && (w = Oe(w, l)), typeof w == "number" && (w = `${w}px`), w;
  }
...
const U = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, Y = "__vueuse_ssr_handlers__", ke = Le();
function Le() {
...
function je(e, t, n, d = {}) {
  ...
  if (!n) return w;
  const x = E(t), _ = He(x), R = (v = d.serializer) != null ? v : $e[_], { pause: C, resume: k } = be(w, () => V(w.value), { flush: m, deep: c, eventFilter: f });
  P(S, () => g(), { flush: m });
  let L = false;
  const D = (h) => {
    l && !L || g(h);
...
function he(e, t, n = {}) {
  const { window: d = W, ...v } = n;
  let m;
  const c = Q(() => d && "ResizeObserver" in d), s = () => {
    m && (m.disconnect(), m = void 0);
  }, a = N(() => {
    const r = E(e);
...
function Be(e, t, n = {}) {
  const { root: d, rootMargin: v = "0px", threshold: m = 0, window: c = W, immediate: s = true } = n, a = Q(() => c && "IntersectionObserver" in c), i = N(() => {
    const l = E(e);
...
function qe(e, t = {}) {
  ...
  const _ = y(false), R = ie({ left: true, right: false, top: true, bottom: false }), C = ie({ left: false, right: false, top: false, bottom: false }), k = (p) => {
    ...
    const X = Math.abs(O * b) <= (c.left || 0), te = Math.abs(O * b) + M.clientWidth >= M.scrollWidth - (c.right || 0) - ue;
    g === "flex" && T === "row-reverse" ? (R.left = te, R.right = X) : (R.left = X, R.right = te), o.value = O;
    let z = M.scrollTop;
  ...
  return A(e, "scroll", n ? Se(F, n, true, false) : F, a), $(() => {
    try {
      const p = E(e);
      if (!p) return;
      D(p);
    ...
    const p = E(e);
    p && D(p);
  }, { attributes: true, childList: true, subtree: true }), A(e, "scrollend", k, a), { x: w, y: S, isScrolling: _, arrivedState: R, directions: C, measure() {
    const p = E(e);
    u && p && D(p);
  } };
}
...
function it(e, t = {}) {
  ...
  const k = () => {
    ...
    L(), !(!G && !re || !f.value) && (S = true, D(), l == null ? void 0 : l(), f.value.close(g, T), f.value = void 0);
  }, p = (g, T = true) => !f.value || r.value !== "OPEN" ? (T && _.push(g), false) : (k(), f.value.send(g), true), V = () => {
    if (S || typeof o.value > "u") return;
    const g = new WebSocket(o.value, i);
    f.value = g, r.value = "CONNECTING", g.onopen = () => {
      r.value = "OPEN", x = 0, n == null ? void 0 : n(g), w == null ? void 0 : w(), k();
    }, g.onclose = (T) => {
      if (r.value = "CLOSED", D(), l == null ? void 0 : l(), d == null ? void 0 : d(g, T), !S && t.autoReconnect && (f.value == null || g === f.value)) {
        const { retries: h = -1, delay: b = 1e3, onFailed: O } = K(t.autoReconnect);
      ...
      if (t.heartbeat) {
        D();
        const { message: h = ae, responseMessage: b = h } = K(t.heartbeat);
  ...
  if (t.heartbeat) {
    const { message: g = ae, interval: T = 1e3, pongTimeout: h = 1e3 } = K(t.heartbeat), { pause: b, resume: O } = Te(() => {
