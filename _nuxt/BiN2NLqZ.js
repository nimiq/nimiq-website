...
import { bD as G, a as M, aW as _, aX as R, bE as K, am as H, bF as X, d as I, aC as J, aD as Z, B as b, o as y, L as Q, C as r, N as ee, l as te, r as E, K as se, x as oe, h as f, H as z, A as ne, O as ie, M as A, g as L, f as j, G as S, F as ae, I as N, e as $, E as P, D as re } from "./B8biU-UE.js";
...
class pe {
  constructor() {
    __publicField(this, "_data", new D());
    __publicField(this, "_hash", new D([...de]));
  ...
  _doProcessBlock(s, t) {
    ...
    for (let d = 0; d < 64; d++) {
      ...
      const g = c & w ^ ~c & v, U = o & i ^ o & l ^ i & l, m = (o << 30 | o >>> 2) ^ (o << 19 | o >>> 13) ^ (o << 10 | o >>> 22), n = (c << 26 | c >>> 6) ^ (c << 21 | c >>> 11) ^ (c << 7 | c >>> 25), k = B + n + g + me[d] + q[d], x = m + U;
  ...
  _append(s) {
    typeof s == "string" && (s = D.fromUtf8(s)), this._data.concat(s), this._nDataBytes += s.sigBytes;
  }
  _process(s) {
...
function he(a, s, t) {
  ...
  const { server: p, lazy: c, default: w, transform: v, pick: B, watch: d, immediate: g, getCachedData: U, deep: m, dedupe: n, ...k } = e, x = R({ ...K, ...k, cache: typeof e.cache == "boolean" ? void 0 : e.cache }), u = { server: p, lazy: c, default: w, transform: v, pick: B, immediate: g, getCachedData: U, deep: m, dedupe: n, watch: d === false ? [] : [...d || [], x] };
  let h;
  return H(d === false ? l.value : l, () => {
    var _a;
    ...
    const O = _(e.timeout);
    let T;
    return O && (T = setTimeout(() => h.abort(new DOMException("Request aborted due to timeout.", "AbortError")), O), h.signal.onabort = () => clearTimeout(T)), (e.$fetch || globalThis.$fetch)(i.value, { signal: h.signal, ...x }).finally(() => {
      clearTimeout(T);
    });
  }, u);
}
...
const ge = { flex: "~ gap-x-16 items-start", group: "", "ml--8": "", "p-8": "", "rounded-2": "", "focus-within:bg-neutral-200": "" }, be = ["name", "required"], _e = { text: "left 14 lg:16 neutral-700", "font-normal": "", "max-w-40ch": "" }, we = I({ __name: "Checkbox", props: J({ name: {}, required: { type: Boolean, default: false } }, { modelValue: { type: Boolean, required: true }, modelModifiers: {} }), emits: ["update:modelValue"], setup(a) {
...
const xe = { pt: "148 md:153 lg:160", "data-slice-type": "newsletter-form" }, Be = { px: "32 md:36 lg:40", "mx-auto": "", "py-60": "", "rounded-6": "", "bg-neutral-0": "", "max-w-65ch": "", shadow: "" }, qe = { flex: "~ col gap-y-20 lg:gap-y-24" }, ke = { grid: "~ cols-2 gap-y-4 gap-x-16" }, Ce = { grid: "~ cols-2 gap-y-4 gap-x-16" }, Se = { key: 0 }, Ve = { key: 1, "text-18": "", "i-nimiq:spinner": "" }, De = { key: 1 }, Ue = { "px-12": "", "py-10": "", "rounded-6": "", "f-mt-xs": "" }, Me = { key: 2, flex: "~ items-start gap-8", text: "f-lg green", "font-semibold": "", "f-mt-md": "" }, Oe = I({ __name: "index", props: le(), setup(a) {
  const s = F("email", ""), t = F("products", ""), e = F("topics", ""), o = E(s.value), i = E(false), l = R(Object.values(W).map((m) => ({ label: m, model: E(e.value.includes(m)) }))), p = R(Object.values(Y).map((m) => ({ label: m, model: E(t.value.includes(m)) }))), c = M(() => l.filter((m) => m.model).map((m) => m.label)), w = M(() => p.filter((m) => m.model).map((m) => m.label)), v = M(() => new URLSearchParams({ email: o.value ?? "", communicationPermission: String(i.value ?? false), topics: c.value.join(","), products: w.value.join(",") })), B = new URL("/api/newsletter/subscribe", se().public.apiDomain), { execute: d, status: g, error: U } = he(B.href, { method: "POST", body: v, watch: false, immediate: false }, "$MhBSkS8Nmy");
  return (m, n) => {
    const k = ae, x = ve;
    return y(), b("section", xe, [r("div", Be, [n[15] || (n[15] = r("h1", { style: { "--nq-font-size-min": "24", "--nq-font-size-max": "28" }, "text-left": "" }, " Nimiq newsletter signup ", -1)), f(g) === "idle" || f(g) === "pending" ? (y(), b(z, { key: 0 }, [n[11] || (n[11] = r("p", { mt: "12 lg:16" }, " You don't want to miss any news about the Nimiq ecosystem? Subscribe to our newsletter and receive regular updates on a variety of Nimiq related topics. ", -1)), r("form", { flex: "~ col gap-y-36 lg:gap-y-40", mt: "36 lg:40", onSubmit: n[2] || (n[2] = ne(() => f(d)(), ["prevent"])) }, [r("fieldset", null, [n[3] || (n[3] = r("legend", { text: "14 neutral", "ml--2": "", "nq-label": "" }, " Your email* ", -1)), Q(r("input", { "onUpdate:modelValue": n[0] || (n[0] = (u) => A(o) ? o.value = u : null), type: "email", placeholder: "Email address", mt: "12 lg:16", required: "", "rounded-full": "", "nq-input-box": "" }, null, 512), [[ie, f(o)]])]), r("fieldset", null, [n[6] || (n[6] = r("legend", { text: "14 neutral", "ml--2": "", "nq-label": "" }, " Communication permission* ", -1)), n[7] || (n[7] = r("p", { mt: "12 lg:16" }, " In order to receive the Nimiq newsletter, you must give us permission to contact you. ", -1)), L(x, { modelValue: f(i), "onUpdate:modelValue": n[1] || (n[1] = (u) => A(i) ? i.value = u : null), required: true, mt: "12 lg:16" }, { default: j(() => [n[5] || (n[5] = S(" Yes, I would like to subscribe to the free Nimiq.com newsletter and receive information tailored to my needs at the email address I have provided based on my registration data and newsletter preferences. I hereby accept the ")), L(k, { to: "/privacy-policy", "text-blue": "", target: "_blank" }, { default: j(() => n[4] || (n[4] = [S(" Privacy Policy. ")])), _: 1, __: [4] })]), _: 1, __: [5] }, 8, ["modelValue"])]), r("fieldset", qe, [n[8] || (n[8] = r("legend", { text: "14 neutral", "ml--2": "", "nq-label": "" }, " Your interests ", -1)), n[9] || (n[9] = r("p", { "mt-20": "", "lg:mt-24": "" }, " In which topics are you interested? ", -1)), r("div", ke, [(y(true), b(z, null, N(f(l), (u) => (y(), $(x, { key: u.label, modelValue: u.model, "onUpdate:modelValue": (h) => u.model = h }, { default: j(() => [S(P(u.label), 1)]), _: 2 }, 1032, ["modelValue", "onUpdate:modelValue"]))), 128))]), n[10] || (n[10] = r("p", { "mt-12": "" }, " Which Nimiq products do you use? ", -1)), r("div", Ce, [(y(true), b(z, null, N(f(p), (u) => (y(), $(x, { key: u.label, modelValue: u.model, "onUpdate:modelValue": (h) => u.model = h }, { default: j(() => [S(P(u.label), 1)]), _: 2 }, 1032, ["modelValue", "onUpdate:modelValue"]))), 128))])]), r("button", { class: re({ "nq-arrow": f(g) === "idle" }), "self-end": "", "nq-pill-lg": "", "nq-pill-blue": "" }, [f(g) === "idle" ? (y(), b("span", Se, " Subscribe ")) : (y(), b("div", Ve))], 2)], 32)], 64)) : f(g) === "error" ? (y(), b("div", De, [n[12] || (n[12] = r("div", { flex: "~ items-center gap-8", text: "f-lg red", "font-semibold": "", "f-mt-md": "" }, [r("div", { "scale-70": "", "i-nimiq:cross": "" }), S(" Error ")], -1)), n[13] || (n[13] = r("p", { "nt-mt-12": "" }, " There was an error while subscribing to the newsletter. Please try again later. ", -1)), r("pre", Ue, "          " + P(f(U)) + `
        `, 1)])) : f(g) === "success" ? (y(), b("div", Me, [n[14] || (n[14] = r("div", { "mt-6": "", "scale-70": "", "i-nimiq:check": "" }, null, -1)), S(" Please confirm your subscription sent to your " + P(f(o)) + ". ", 1)])) : oe("", true)])]);
  };
} }), Fe = ue(Oe, [["__scopeId", "data-v-88ce6287"]]);
export {
  Fe as default
};
