import { t as j, n as ce, d as B, e as pe, m as we, f as G, g as be, h as q, p as J, j as fe, k as $, l as ge, o as ye, q as Se, r as Ee, s as Oe, v as xe, x as _e, a as Re, y as re } from "./BPgVFFXt.js";
import { i as N, b1 as E, R as y, ae as k, bk as de, d as se, E as Te, ak as ve, a1 as Me, aR as ie, bl as We, B as Z, bm as Ce, as as Ie, u as Ne, N as Ve } from "./C95QRNzc.js";
function Ue(e = {}) {
  ...
  return we({ define: d, reuse: v }, [d, v]);
}
function Ae(e) {
  const t = {};
...
function ee(e, t, n = {}) {
  ...
  let m;
  const c = K(() => d && "MutationObserver" in d), s = () => {
...
function Ke(e, t = {}) {
  ...
  return n && r(), q(f), { isActive: de(c), pause: f, resume: r };
}
const ke = Symbol("vueuse-ssr-width");
function me() {
  const e = We() ? Se(ke, null) : null;
  return typeof e == "number" ? e : void 0;
}
function H(e, t = {}) {
  const { window: n = W, ssrWidth: d = me() } = t, v = K(() => n && "matchMedia" in n && typeof n.matchMedia == "function"), m = y(typeof d == "number"), c = y(), s = y(false), a = (i) => {
    s.value = i.matches;
  };
  return Me(() => {
    if (m.value) {
...
function Xe(e, t = {}) {
  function n(o, l) {
    let w = E(e[E(o)]);
    return l != null && (w = Ee(w, l)), typeof w == "number" && (w = `${w}px`), w;
  }
...
const U = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, Y = "__vueuse_ssr_handlers__", Pe = Le();
function Le() {
...
function je(e, t, n, d = {}) {
  ...
  if (!n) return w;
  const x = E(t), _ = He(x), T = (v = d.serializer) != null ? v : $e[_], { pause: C, resume: P } = Oe(w, () => V(w.value), { flush: m, deep: c, eventFilter: f });
  k(S, () => g(), { flush: m });
  let L = false;
  const D = (h) => {
    l && !L || g(h);
  ...
  function M(h) {
    ...
    if (b == null) return a && x != null && n.setItem(S.value, T.write(x)), x;
    if (!h && i) {
      const O = T.read(b);
  ...
  function g(h) {
    if (!(h && h.storageArea !== n)) {
      ...
      if (!(h && h.key !== S.value)) {
        C();
        try {
          (h == null ? void 0 : h.newValue) !== T.write(w.value) && (w.value = M(h));
        } catch (b) {
          o(b);
        } finally {
          h ? Ve(P) : P();
...
function Ze(e, t, n = {}) {
  ...
  return m && ee(s, a, { attributeFilter: ["style", "class"], window: d }), k([s, () => E(e)], (i, u) => {
    u[0] && u[1] && u[0].style.removeProperty(u[1]), a();
  }, { immediate: true }), k([c, s], ([i, u]) => {
    const r = E(e);
...
function et(e = {}) {
  ...
  let v = B;
  return t && (v = fe(d, () => n.value = t.devicePixelRatio)), { pixelRatio: de(n), stop: v };
}
function tt(e = {}) {
...
function he(e, t, n = {}) {
  const { window: d = W, ...v } = n;
  let m;
  const c = K(() => d && "ResizeObserver" in d), s = () => {
    m && (m.disconnect(), m = void 0);
  }, a = N(() => {
    const r = E(e);
...
function nt(e, t = {}) {
  ...
  return he(e, S), k(() => I(e), (x) => !x && S()), ee(e, S, { attributeFilter: ["style", "class"] }), v && A("scroll", S, { capture: true, passive: true }), d && A("resize", S, { passive: true }), $(() => {
    m && S();
...
function Be(e, t, n = {}) {
  const { root: d, rootMargin: v = "0px", threshold: m = 0, window: c = W, immediate: s = true } = n, a = K(() => c && "IntersectionObserver" in c), i = N(() => {
    const l = E(e);
    return j(l).map(I).filter(ce);
  });
  let u = B;
  const r = y(s), f = a.value ? k(() => [i.value, I(d), r.value], ([l, w]) => {
...
function qe(e, t = {}) {
  ...
  function x(p, V) {
    ...
    const b = E(e);
    if (!b) return;
    (R = b instanceof Document ? u.document.body : b) == null || R.scrollTo({ top: (M = E(V)) != null ? M : S.value, left: (g = E(p)) != null ? g : w.value, behavior: E(i) });
  ...
  const _ = y(false), T = ie({ left: true, right: false, top: true, bottom: false }), C = ie({ left: false, right: false, top: false, bottom: false }), P = (p) => {
    _.value && (_.value = false, C.left = false, C.right = false, C.top = false, C.bottom = false, v(p));
  }, L = ge(P, n + d), D = (p) => {
    var V;
    if (!u) return;
    const M = ((V = p == null ? void 0 : p.document) == null ? void 0 : V.documentElement) || (p == null ? void 0 : p.documentElement) || I(p), { display: g, flexDirection: R, direction: h } = getComputedStyle(M), b = h === "rtl" ? -1 : 1, O = M.scrollLeft;
    C.left = O < o.value, C.right = O > o.value;
    const Q = Math.abs(O * b) <= (c.left || 0), te = Math.abs(O * b) + M.clientWidth >= M.scrollWidth - (c.right || 0) - ue;
    g === "flex" && R === "row-reverse" ? (T.left = te, T.right = Q) : (T.left = Q, T.right = te), o.value = O;
    let z = M.scrollTop;
    ...
    g === "flex" && R === "column-reverse" ? (T.top = oe, T.bottom = ne) : (T.top = ne, T.bottom = oe), l.value = z;
  }, F = (p) => {
    var V;
    if (!u) return;
    const M = (V = p.target.documentElement) != null ? V : p.target;
    D(M), _.value = true, L(p), m(p);
  };
  return A(e, "scroll", n ? ye(F, n, true, false) : F, a), $(() => {
    try {
      const p = E(e);
      if (!p) return;
      D(p);
    ...
    const p = E(e);
    p && D(p);
  }, { attributes: true, childList: true, subtree: true }), A(e, "scrollend", P, a), { x: w, y: S, isScrolling: _, arrivedState: T, directions: C, measure() {
    const p = E(e);
    u && p && D(p);
  } };
}
...
function it(e, t = {}) {
  ...
  const P = () => {
    ...
    L(), !(!G && !re || !f.value) && (S = true, D(), l == null ? void 0 : l(), f.value.close(g, R), f.value = void 0);
  }, p = (g, R = true) => !f.value || r.value !== "OPEN" ? (R && _.push(g), false) : (P(), f.value.send(g), true), V = () => {
    if (S || typeof o.value > "u") return;
    const g = new WebSocket(o.value, i);
    f.value = g, r.value = "CONNECTING", g.onopen = () => {
      r.value = "OPEN", x = 0, n == null ? void 0 : n(g), w == null ? void 0 : w(), P();
    }, g.onclose = (R) => {
      if (r.value = "CLOSED", D(), l == null ? void 0 : l(), d == null ? void 0 : d(g, R), !S && t.autoReconnect && (f.value == null || g === f.value)) {
        const { retries: h = -1, delay: b = 1e3, onFailed: O } = X(t.autoReconnect);
        (typeof h == "function" ? h : () => typeof h == "number" && (h < 0 || x < h))(x) ? (x += 1, T = setTimeout(V, b)) : O == null ? void 0 : O();
      }
    }, g.onerror = (R) => {
      v == null ? void 0 : v(g, R);
    }, g.onmessage = (R) => {
      if (t.heartbeat) {
        ...
        if (R.data === E(b)) return;
      }
      u.value = R.data, m == null ? void 0 : m(g, R);
    };
  };
  if (t.heartbeat) {
    const { message: g = ae, interval: R = 1e3, pongTimeout: h = 1e3 } = X(t.heartbeat), { pause: b, resume: O } = Re(() => {
  ...
  return c && M(), s && k(o, M), { data: u, status: r, close: F, send: p, open: M, ws: f };
}
function lt(e = {}) {
