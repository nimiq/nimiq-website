import { aC as $, aD as P, aE as q, aF as M, aG as R, aH as b, aI as O, M as I, q as L, j as p, C as U, a7 as C, ay as H, G as B, c as D, R as G, o as V, m as k, S as F } from "./BguRs3jF.js";
async function T(e, t) {
  return await J(t).catch((i) => (console.error("Failed to get image meta for " + t, i + ""), { width: 0, height: 0, ratio: 0 }));
}
async function J(e) {
...
function X(e = {}) {
  const t = e.formatter, r = e.keyMap && typeof e.keyMap != "function" ? _(e.keyMap) : e.keyMap, i = {};
  for (const s in e.valueMap) {
    const n = s, a = e.valueMap[n];
    i[n] = typeof a == "object" ? _(a) : a;
  }
  return (s) => {
    const n = [];
    for (const a in s) {
...
function Y(e) {
  const t = { options: e }, r = (s, n = {}) => W(t, s, n), i = (s, n, a) => r(s, $({ modifiers: n }, a)).url;
...
function W(e, t, r) {
  ...
  if (t = q(t) ? t : P(t), !n.supportsAlias) {
    for (const m in e.options.alias) if (t.startsWith(m)) {
      const g = e.options.alias[m];
      g && (t = M(g, t.slice(m.length)));
    }
  }
  if (n.validateDomains && q(t)) {
    const m = R(t).host;
    if (!e.options.domains.find((g) => g === m)) return { url: t };
  }
  const o = $(r, a, s), d = { ...o, modifiers: { ...o.modifiers, width: ((_a = o.modifiers) == null ? void 0 : _a.width) ? y(o.modifiers.width) : void 0, height: ((_b = o.modifiers) == null ? void 0 : _b.height) ? y(o.modifiers.height) : void 0 } }, u = n.getImage(t, d, e);
  return u.format || (u.format = d.modifiers.format || ""), u;
}
function te(e, t) {
...
function ie(e, t, r) {
  var _a, _b, _c, _d, _e;
  const i = y((_a = r.modifiers) == null ? void 0 : _a.width), s = y((_b = r.modifiers) == null ? void 0 : _b.height), n = Q(r.sizes), a = ((_c = r.densities) == null ? void 0 : _c.trim()) ? K(r.densities.trim()) : e.options.densities;
  Z(a);
  const o = i && s ? s / i : 0, d = [], u = [];
  if (Object.keys(n).length >= 1) {
    for (const l in n) {
      const h = x(l, String(n[l]), s, o, e);
      if (h !== void 0) {
        d.push({ size: h.size, screenMaxWidth: h.screenMaxWidth, media: `(max-width: ${h.screenMaxWidth}px)` });
        for (const f of a) u.push({ width: h._cWidth * f, src: z(e, t, r, h, f) });
      }
    }
    se(d);
    ...
    f === void 0 && (f = { size: "", screenMaxWidth: 0, _cWidth: (_d = r.modifiers) == null ? void 0 : _d.width, _cHeight: (_e = r.modifiers) == null ? void 0 : _e.height }), u.push({ width: l, src: z(e, t, r, f, l) });
  }
  ne(u);
  const m = u[u.length - 1], g = d.length ? d.map((l) => `${l.media ? l.media + " " : ""}${l.size}`).join(", ") : void 0, w = g ? "w" : "x", S = u.map((l) => `${l.src} ${l.width}${w}`).join(", ");
...
const ae = X({ keyMap: { format: "f", fit: "fit", width: "w", height: "h", resize: "s", quality: "q", background: "b" }, formatter: (e, t) => b(e) + "_" + b(t.toString()) }), ce = oe({ validateDomains: true, supportsAlias: true, getImage(e, { modifiers: t, baseURL: r }, i) {
  t.width && t.height && (t.resize = `${t.width}x${t.height}`, delete t.width, delete t.height);
  const s = ae(t) || "_";
  return r || (r = M(i.options.nuxt.baseURL, "/_ipx")), { url: M(r, s, O(e).replace(/\/{2,}/g, "/")) };
...
const ue = (e) => {
  ...
  const r = e, i = t, s = A(), { providerOptions: n, normalizedAttrs: a, imageModifiers: o } = ue(r), d = p(() => s.getSizes(r.src, { ...n.value, sizes: r.sizes, densities: r.densities, modifiers: o.value })), u = U(false), m = C(), g = p(() => ({ ...a.value, "data-nuxt-img": "", ...!r.placeholder || u.value ? { sizes: d.value.sizes, srcset: d.value.srcset } : {}, ...m })), w = p(() => {
    if (u.value) return false;
    const c = r.placeholder === "" ? [10, 10] : r.placeholder;
    if (!c) return false;
    if (typeof c == "string") return c;
    const [v = 10, j = v, E = 50, N = 3] = Array.isArray(c) ? c : typeof c == "number" ? [c] : [];
    return s(r.src, { ...o.value, width: v, height: j, quality: E, blur: N }, n.value);
  }), S = p(() => r.sizes ? d.value.src : s(r.src, o.value, n.value)), l = p(() => w.value || S.value), h = I().isHydrating, f = H("imgEl");
  return B(() => {
    if (w.value || r.custom) {
      const c = new Image();
      S.value && (c.src = S.value), r.sizes && (c.sizes = d.value.sizes || "", c.srcset = d.value.srcset), c.onload = (v) => {
        u.value = true, i("load", v);
      }, c.onerror = (v) => {
        i("error", v);
      }, le("nuxt-image");
      return;
  ...
  }), (c, v) => e.custom ? G(c.$slots, "default", F(k({ key: 1 }, { imgAttrs: g.value, isLoaded: u.value, src: l.value }))) : (V(), D("img", k({ key: 0, ref_key: "imgEl", ref: f, class: w.value ? e.placeholderClass : void 0 }, g.value, { src: l.value }), null, 16, fe));
} };
export {
  ge as _
};
