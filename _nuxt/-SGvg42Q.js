import { n as B, t as q, i as G, c as le, d as H, e as we, b as ge, p as J, f as $, g as ae, h as te, j as be, k as ye, m as Se, l as Ee, o as Oe, q as Te, r as Ce, s as ue, v as xe } from "./8Lad1M9Y.js";
import { j as k, P as E, L as P, E as _e, a0 as ce, B as X, au as T, V as Re, aO as Me, u as ke, d as ne, aj as oe, aP as de, aQ as We, aR as Ae, a1 as De } from "./7tyaNgEA.js";
function Ke(e = {}) {
...
function fe(e, t, n = {}) {
  ...
  let m;
  const a = Q(() => c && "MutationObserver" in c), r = () => {
...
function j(e, t = {}) {
  const { window: n = W, ssrWidth: c = ve() } = t, v = Q(() => n && "matchMedia" in n && typeof n.matchMedia == "function"), m = E(typeof c == "number"), a = E(), r = E(false), u = (i) => {
    r.value = i.matches;
  };
  return Re(() => {
    if (m.value) {
...
function Ze(e, t = {}) {
  function n(s, l) {
    let w = T(e[T(s)]);
    return l != null && (w = Ce(w, l)), typeof w == "number" && (w = `${w}px`), w;
  }
...
function $e(e = {}) {
  const { selector: t = "html", attribute: n = "class", initialValue: c = "auto", window: v = W, storage: m, storageKey: a = "vueuse-color-scheme", listenToStorageChanges: r = true, storageRef: u, emitAuto: i, disableTransition: d = true } = e, o = { auto: "", light: "light", dark: "dark", ...e.modes || {} }, f = Fe({ window: v }), s = k(() => f.value ? "dark" : "light"), l = u || (a == null ? le(c) : he(a, c, m, { window: v, listenToStorageChanges: r })), w = k(() => l.value === "auto" ? s.value : l.value), S = me("updateHTMLAttrs", (_, V, L) => {
    ...
    let A;
    d && (A = v.document.createElement("style"), A.appendChild(document.createTextNode(Be)), v.document.head.appendChild(A));
...
function pe(e, t, n = {}) {
  const { window: c = W, ...v } = n;
  let m;
  const a = Q(() => c && "ResizeObserver" in c), r = () => {
    m && (m.disconnect(), m = void 0);
  }, u = k(() => {
    const o = T(e);
...
function qe(e, t, n = {}) {
  const { root: c, rootMargin: v = "0px", threshold: m = 0, window: a = W, immediate: r = true } = n, u = Q(() => a && "IntersectionObserver" in a), i = k(() => {
    const l = T(e);
...
const se = 1;
function Ge(e, t = {}) {
  const { throttle: n = 0, idle: c = 200, onStop: v = B, onScroll: m = B, offset: a = { left: 0, right: 0, top: 0, bottom: 0 }, eventListenerOptions: r = { capture: false, passive: true }, behavior: u = "auto", window: i = W, onError: d = (p) => {
    console.error(p);
  ...
  const S = E(false), b = oe({ left: true, right: false, top: true, bottom: false }), O = oe({ left: false, right: false, top: false, bottom: false }), N = (p) => {
    ...
    D === "flex" && A === "column-reverse" ? (b.top = ee, b.bottom = Z) : (b.top = Z, b.bottom = ee), f.value = F;
  }, L = (p) => {
    var R;
    if (!i) return;
  ...
  return z(e, "scroll", n ? Oe(L, n, true, false) : L, r), H(() => {
    try {
      const p = T(e);
      if (!p) return;
      V(p);
...
function at(e, t = {}) {
  ...
  if (t.heartbeat) {
    const { message: g = ie, interval: y = 1e3, pongTimeout: h = 1e3 } = Y(t.heartbeat), { pause: C, resume: M } = ge(() => {
  ...
  return a && A(), r && P(s, A), { data: d, status: o, close: R, send: x, open: A, ws: f };
}
function ut(e = {}) {
