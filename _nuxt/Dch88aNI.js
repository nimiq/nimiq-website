...
*/
const de = Symbol("Comlink.proxy"), ve = Symbol("Comlink.endpoint"), Ee = Symbol("Comlink.releaseProxy"), G = Symbol("Comlink.finalizer"), $ = Symbol("Comlink.thrown"), fe = (s) => typeof s == "object" && s !== null || typeof s == "function", pe = /* @__PURE__ */ new Map([["proxy", { canHandle: (s) => fe(s) && s[de], serialize(s) {
  const { port1: e, port2: t } = new MessageChannel();
  return ge(s, e), [t, [t]];
}, deserialize: (s) => (s.start(), he(s)) }], ["throw", { canHandle: (s) => fe(s) && $ in s, serialize({ value: s }) {
  let e;
  return e = s instanceof Error ? { isError: true, value: { message: s.message, name: s.name, stack: s.stack } } : { isError: false, value: s }, [e, []];
}, deserialize(s) {
  throw s.isError ? Object.assign(new Error(s.value.message), s.value) : s.value;
} }]]);
function ge(s, e = globalThis, t = ["*"]) {
  e.addEventListener("message", function r(n) {
    ...
    const { id: a, type: o, path: c } = Object.assign({ path: [] }, n.data), l = (n.data.argumentList || []).map(B);
    let _;
    try {
      const f = c.slice(0, -1).reduce((v, N) => v[N], s), b = c.reduce((v, N) => v[N], s);
      switch (o) {
        case "GET":
          _ = b;
          break;
    ...
    Promise.resolve(_).catch((f) => ({ value: f, [$]: 0 })).then((f) => {
      const [b, v] = ee(f);
      e.postMessage(Object.assign(Object.assign({}, b), { id: a }), v), o === "RELEASE" && (e.removeEventListener("message", r), ke(e), G in s && typeof s[G] == "function" && s[G]());
    }).catch((f) => {
      const [b, v] = ee({ value: new TypeError("Unserializable return value"), [$]: 0 });
      e.postMessage(Object.assign(Object.assign({}, b), { id: a }), v);
    });
  }), e.start && e.start();
}
function ke(s) {
...
function Se(s) {
  return M(s, { type: "RELEASE" }).then(() => {
    ke(s);
  });
}
const J = /* @__PURE__ */ new WeakMap(), Y = "FinalizationRegistry" in globalThis && new FinalizationRegistry((s) => {
  const e = (J.get(s) || 0) - 1;
  J.set(s, e), e === 0 && Se(s);
});
function ce(s, e = [], t = function() {
}) {
  let r = false;
  const n = new Proxy(t, { get(a, o) {
    if (Z(r), o === Ee) return () => {
      (function(c) {
        Y && Y.unregister(c);
      })(n), Se(s), r = true;
    ...
    if (o === "then") {
      ...
      const c = M(s, { type: "GET", path: e.map((l) => l.toString()) }).then(B);
    ...
    Z(r);
    const [l, _] = ee(c);
    return M(s, { type: "SET", path: [...e, o].map((f) => f.toString()), value: l }, _).then(B);
  }, apply(a, o, c) {
    Z(r);
    const l = e[e.length - 1];
    if (l === ve) return M(s, { type: "ENDPOINT" }).then(B);
    if (l === "bind") return ce(s, e.slice(0, -1));
    const [_, f] = we(c);
    return M(s, { type: "APPLY", path: e.map((b) => b.toString()), argumentList: _ }, f).then(B);
  }, construct(a, o) {
    Z(r);
    const [c, l] = we(o);
    return M(s, { type: "CONSTRUCT", path: e.map((_) => _.toString()), argumentList: c }, l).then(B);
  } });
  return function(a, o) {
    const c = (J.get(o) || 0) + 1;
    J.set(o, c), Y && Y.register(a, o, a);
  }(n, s), n;
}
function we(s) {
  const e = s.map(ee);
...
function Te(s) {
  return Object.assign(s, { [de]: true });
}
function Oe(s, e = globalThis, t = "*") {
  return { postMessage: (r, n) => s.postMessage(r, t, n), addEventListener: e.addEventListener.bind(e), removeEventListener: e.removeEventListener.bind(e) };
}
function ee(s) {
  for (const [e, t] of pe) if (t.canHandle(s)) {
    const [r, n] = t.serialize(s);
    return [{ type: "HANDLER", name: e, value: r }, n];
  }
  return [{ type: "RAW", value: s }, ze.get(s) || []];
}
function B(s) {
  switch (s.type) {
    case "HANDLER":
      return pe.get(s.name).deserialize(s.value);
    case "RAW":
      return s.value;
  }
}
function M(s, e, t) {
  return new Promise((r) => {
    ...
    s.addEventListener("message", function a(o) {
      o.data && o.data.id && o.data.id === n && (s.removeEventListener("message", a), r(o.data));
    }), s.start && s.start(), s.postMessage(Object.assign({ id: n }, e), t);
  });
}
const Ne = Object.freeze(Object.defineProperty({ __proto__: null, createEndpoint: ve, expose: ge, finalizer: G, proxy: Te, proxyMarker: de, releaseProxy: Ee, transfer: Ue, transferHandlers: pe, windowEndpoint: Oe, wrap: he }, Symbol.toStringTag, { value: "Module" }));
let i, d = 0, X = null;
function j() {
...
} }, Be = typeof Q.encodeInto == "function" ? function(s, e) {
  return Q.encodeInto(s, e);
} : function(s, e) {
  const t = Q.encode(s);
...
function K(s, e, t) {
  if (t === void 0) {
    const c = Q.encode(s), l = e(c.length, 1) >>> 0;
  ...
  if (o !== r) {
    o !== 0 && (s = s.slice(o)), n = t(n, r, r = o + s.length * 3, 1) >>> 0;
    const c = j().subarray(n + o, n + r), l = Be(s, c);
...
function U(s, e) {
  return s = s >>> 0, xe.decode(j().subarray(s, s + e));
}
function C(s) {
  return s == null;
}
...
function g(s, e) {
  const t = e(s.length * 1, 1) >>> 0;
  return j().set(s, t / 1), d = s.length, t;
}
function p(s) {
  const e = i.__wbindgen_export_4.get(s);
  return i.__externref_table_dealloc(s), e;
}
function y(s, e) {
  return s = s >>> 0, j().subarray(s / 1, s / 1 + e);
}
const re = typeof FinalizationRegistry > "u" ? { register: () => {
}, unregister: () => {
} } : new FinalizationRegistry((s) => i.__wbg_address_free(s >>> 0, 1));
class k {
  static __wrap(e) {
    e = e >>> 0;
...
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_blskeypair_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_blspublickey_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_blssecretkey_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_clientconfiguration_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_commitment_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_commitmentpair_free(s >>> 0, 1));
const De = typeof FinalizationRegistry > "u" ? { register: () => {
}, unregister: () => {
} } : new FinalizationRegistry((s) => i.__wbg_cryptoutils_free(s >>> 0, 1));
class F {
  __destroy_into_raw() {
    const e = this.__wbg_ptr;
  ...
  static computeHmacSha512(e, t) {
    const r = g(e, i.__wbindgen_malloc), n = d, a = g(t, i.__wbindgen_malloc), o = d, c = i.cryptoutils_computeHmacSha512(r, n, a, o);
    var l = y(c[0], c[1]).slice();
    return i.__wbindgen_free(c[0], c[1] * 1, 1), l;
  }
  static computePBKDF2sha512(e, t, r, n) {
    const a = g(e, i.__wbindgen_malloc), o = d, c = g(t, i.__wbindgen_malloc), l = d, _ = i.cryptoutils_computePBKDF2sha512(a, o, c, l, r, n);
    if (_[3]) throw p(_[2]);
    var f = y(_[0], _[1]).slice();
    return i.__wbindgen_free(_[0], _[1] * 1, 1), f;
  }
}
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_es256publickey_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_es256signature_free(s >>> 0, 1));
const Le = typeof FinalizationRegistry > "u" ? { register: () => {
}, unregister: () => {
} } : new FinalizationRegistry((s) => i.__wbg_hash_free(s >>> 0, 1));
class le {
  __destroy_into_raw() {
    const e = this.__wbg_ptr;
  ...
  static computeBlake2b(e) {
    ...
    var a = y(n[0], n[1]).slice();
    return i.__wbindgen_free(n[0], n[1] * 1, 1), a;
  }
  ...
  static computeNimiqArgon2d(e, t, r, n) {
    const a = g(e, i.__wbindgen_malloc), o = d, c = g(t, i.__wbindgen_malloc), l = d, _ = i.hash_computeNimiqArgon2d(a, o, c, l, r, n);
    if (_[3]) throw p(_[2]);
    var f = y(_[0], _[1]).slice();
    return i.__wbindgen_free(_[0], _[1] * 1, 1), f;
  }
  static computeNimiqArgon2id(e, t, r, n) {
    const a = g(e, i.__wbindgen_malloc), o = d, c = g(t, i.__wbindgen_malloc), l = d, _ = i.hash_computeNimiqArgon2id(a, o, c, l, r, n);
    if (_[3]) throw p(_[2]);
    var f = y(_[0], _[1]).slice();
    return i.__wbindgen_free(_[0], _[1] * 1, 1), f;
  }
}
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_hashedtimelockedcontract_free(s >>> 0, 1));
const ne = typeof FinalizationRegistry > "u" ? { register: () => {
}, unregister: () => {
} } : new FinalizationRegistry((s) => i.__wbg_keypair_free(s >>> 0, 1));
class D {
  static __wrap(e) {
    e = e >>> 0;
...
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_merkletree_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_partialsignature_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_policy_free(s >>> 0, 1));
const ie = typeof FinalizationRegistry > "u" ? { register: () => {
}, unregister: () => {
} } : new FinalizationRegistry((s) => i.__wbg_privatekey_free(s >>> 0, 1));
class m {
  static __wrap(e) {
    e = e >>> 0;
...
} } : new FinalizationRegistry((s) => i.__wbg_publickey_free(s >>> 0, 1));
class z {
  static __wrap(e) {
    e = e >>> 0;
  ...
  constructor(e) {
    ...
    if (n[2]) throw p(n[1]);
    return this.__wbg_ptr = n[0] >>> 0, se.register(this, this.__wbg_ptr, this), this;
  }
  serialize() {
...
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_randomsecret_free(s >>> 0, 1));
const me = typeof FinalizationRegistry > "u" ? { register: () => {
}, unregister: () => {
} } : new FinalizationRegistry((s) => i.__wbg_signature_free(s >>> 0, 1));
class R {
  static __wrap(e) {
    e = e >>> 0;
  ...
  static create(e, t, r) {
    S(e, m), S(t, z);
    const n = g(r, i.__wbindgen_malloc), a = d, o = i.signature_create(e.__wbg_ptr, t.__wbg_ptr, n, a);
    return R.__wrap(o);
  }
  static fromAsn1(e) {
...
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_signatureproof_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_stakingcontract_free(s >>> 0, 1));
const ae = typeof FinalizationRegistry > "u" ? { register: () => {
}, unregister: () => {
} } : new FinalizationRegistry((s) => i.__wbg_transaction_free(s >>> 0, 1));
class L {
  static __wrap(e) {
    e = e >>> 0;
  ...
  constructor(e, t, r, n, a, o, c, l, _, f, b) {
    S(e, k);
    var v = C(r) ? 0 : g(r, i.__wbindgen_malloc), N = d;
    S(n, k);
    var Ce = C(o) ? 0 : g(o, i.__wbindgen_malloc), Pe = d;
    const te = i.transaction_new(e.__wbg_ptr, C(t) ? 16777215 : t, v, N, n.__wbg_ptr, C(a) ? 16777215 : a, Ce, Pe, c, l, C(_) ? 16777215 : _, f, b);
    if (te[2]) throw p(te[1]);
    return this.__wbg_ptr = te[0] >>> 0, ae.register(this, this.__wbg_ptr, this), this;
  }
  sign(e) {
    S(e, D);
...
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_transactionbuilder_free(s >>> 0, 1));
typeof FinalizationRegistry > "u" || new FinalizationRegistry((s) => i.__wbg_vestingcontract_free(s >>> 0, 1));
var Me = class {
  static subarray(s, e = 0, t = s.byteLength) {
    function r(a, o, c) {
      return a < o ? o : a > c ? c : a;
    }
    e = r(e, 0, s.byteLength), t = r(t, 0, s.byteLength);
    let n = t - e;
    return n < 0 && (n = 0), new Uint8Array(s.buffer, s.byteOffset + e, n);
  }
}, _e = (_a = class {
  static isInteger(e) {
  ...
  static _codePointAt(e, t) {
    ...
    var n = e.charCodeAt(t), a = e.charAt(t), o = A._isLeadingSurrogate(n), c = A._isTrailingSurrogate(n);
    if (!o && !c) return { "[[CodePoint]]": a, "[[CodeUnitCount]]": 1, "[[IsUnpairedSurrogate]]": false };
    if (c || t + 1 === r) return { "[[CodePoint]]": a, "[[CodeUnitCount]]": 1, "[[IsUnpairedSurrogate]]": true };
    var l = e.charCodeAt(t + 1);
    return A._isTrailingSurrogate(l) ? { "[[CodePoint]]": A._utf16SurrogatePairToCodePoint(n, l), "[[CodeUnitCount]]": 2, "[[IsUnpairedSurrogate]]": false } : { "[[CodePoint]]": a, "[[CodeUnitCount]]": 1, "[[IsUnpairedSurrogate]]": true };
  }
  ...
  readVarUint() {
    ...
    do {
      ...
      r = this.readUint8(), e += t < 28 ? (r & ye) << t : (r & ye) * Math.pow(2, t), t += 7;
    } while (r >= oe);
    return e;
  }
  writeVarUint(e) {
...
}, __publicField(_b, "EMPTY", new _b(0)), _b), oe = 128, ye = 127, He = -128, je = Math.pow(2, 31), w = (_c = class {
  ...
  static _base64encodeChunk(e, t, r) {
    let n;
    const a = [];
    for (let o = t; o < r; o += 3) n = (e[o] << 16 & 16711680) + (e[o + 1] << 8 & 65280) + (e[o + 2] & 255), a.push(_c._tripletToBase64(n));
    return a.join("");
  }
  static _base64fromByteArray(e) {
    ...
    return n === 1 ? (t = e[r - 1], a += _c._BASE64_LOOKUP[t >> 2], a += _c._BASE64_LOOKUP[t << 4 & 63], a += "==") : n === 2 && (t = (e[r - 2] << 8) + e[r - 1], a += _c._BASE64_LOOKUP[t >> 10], a += _c._BASE64_LOOKUP[t >> 4 & 63], a += _c._BASE64_LOOKUP[t << 2 & 63], a += "="), o.push(a), o.join("");
  }
  static toBase64(e) {
  ...
  static toBase32(e, t = _c.BASE32_ALPHABET.NIMIQ) {
    let r = 3, n = 0, a, o, c, l = "";
    for (c = 0; c < e.length; c++) a = e[c], o = n | a >> r, l += t[o & 31], r > 5 && (r -= 5, o = a >> r, l += t[o & 31]), r = 5 - r, n = a << r, r = 8 - r;
  ...
  static fromBase32(e, t = _c.BASE32_ALPHABET.NIMIQ) {
    ...
    return e.toUpperCase().split("").forEach((l) => {
      t.length === 33 && l === t[32] || (n = r[l] & 255, a -= 5, a > 0 ? o |= n << a : a < 0 ? (c.push(o | n >> -a), a += 8, o = n << a & 255) : (c.push(o | n), a = 8, o = 0));
    }), a !== 8 && o !== 0 && c.push(o), new Uint8Array(c);
  }
  static toHex(e) {
    let t = "";
  ...
  static equals(e, t) {
    ...
    for (let a = 0; a < r.length; a++) if (r[a] !== n[a]) return false;
    return true;
  }
  static compare(e, t) {
...
for (let s = 0, e = w.BASE64_ALPHABET.length; s < e; ++s) w._BASE64_LOOKUP[s] = w.BASE64_ALPHABET[s];
var Ae = class Ie {
  equals(e) {
  ...
  static entropyToMnemonic(e, t) {
    ...
    const r = _f._normalizeEntropy(e), n = _f._entropyToBits(r), a = _f._sha256Checksum(r), o = n + a;
  ...
  static getMnemonicType(e, t) {
    ...
    if (n && a) return _f.MnemonicType.UNKNOWN;
    if (!n && !a) throw new Error("Invalid checksum");
  ...
  static _mnemonicToBits(e, t) {
    ...
    return r.map(function(a) {
      const o = t.indexOf(a.toLowerCase());
      if (o === -1) throw new Error(`Invalid mnemonic, word >${a}< is not in wordlist`);
      return o.toString(2).padStart(11, "0");
    }).join("");
  }
  static _bitsToEntropy(e, t = false) {
    const r = e.length - (e.length % 8 || 8), n = e.slice(0, r), a = e.slice(r), o = n.match(/(.{8})/g);
    ...
    if ((t ? _f._crcChecksum(l) : _f._sha256Checksum(l)) !== a) throw new Error("Invalid checksum");
    return l;
  }
  static _salt(e) {
    return `mnemonic${e || ""}`;
...
var W = (_g = class extends Ae {
  ...
  static async exportEncrypted(e, t) {
    ...
    n.write(e.serialize());
    const a = le.computeBlake2b(n).subarray(0, _g.ENCRYPTION_CHECKSUM_SIZE_V3), o = new x(a.byteLength + n.byteLength);
    o.write(a), o.write(n);
  ...
  static async _decryptV3(e, t, r) {
    const n = e.read(_g.ENCRYPTION_SALT_SIZE), a = e.read(_g.ENCRYPTION_CHECKSUM_SIZE_V3 + 4 + _g.SIZE), o = await F.otpKdf(a, t, n, r), c = o.subarray(0, _g.ENCRYPTION_CHECKSUM_SIZE_V3), l = o.subarray(_g.ENCRYPTION_CHECKSUM_SIZE_V3), _ = le.computeBlake2b(l).subarray(0, _g.ENCRYPTION_CHECKSUM_SIZE_V3);
...
const be = Fe(() => new Worker(new URL("" + new URL("crypto-DiM_ROa-.js", import.meta.url).href, import.meta.url)), (s) => he(s));
for (const s in be) {
  const e = be[s];
  typeof e == "function" && (F[s] = e);
}
export {
  k as Address,
  Me as ArrayUtils,
