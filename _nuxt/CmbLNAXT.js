...
*/
const de = Symbol("Comlink.proxy"), ve = Symbol("Comlink.endpoint"), Ee = Symbol("Comlink.releaseProxy"), G = Symbol("Comlink.finalizer"), $ = Symbol("Comlink.thrown"), fe = (a) => typeof a == "object" && a !== null || typeof a == "function", pe = /* @__PURE__ */ new Map([["proxy", { canHandle: (a) => fe(a) && a[de], serialize(a) {
  const { port1: e, port2: t } = new MessageChannel();
  return ge(a, e), [t, [t]];
}, deserialize: (a) => (a.start(), he(a)) }], ["throw", { canHandle: (a) => fe(a) && $ in a, serialize({ value: a }) {
  let e;
...
function Te(a) {
  return Object.assign(a, { [de]: true });
}
...
function ee(a) {
  for (const [e, t] of pe) if (t.canHandle(a)) {
...
function B(a) {
  switch (a.type) {
    case "HANDLER":
      return pe.get(a.name).deserialize(a.value);
    case "RAW":
      return a.value;
  }
}
...
const be = Fe(() => new Worker(new URL("" + new URL("crypto-DiM_ROa-.js", import.meta.url).href, import.meta.url)), (a) => he(a));
for (const a in be) {
  const e = be[a];
