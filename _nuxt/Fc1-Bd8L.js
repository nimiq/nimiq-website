import { b4 as $, b5 as E, b6 as M, b7 as b, b8 as R, b9 as q, ba as O, Z as W, K as L, a as p, r as U, as as B, ay as H, n as C, B as D, l as V, o as F, J as k, a3 as G } from "./B8biU-UE.js";
async function J(e, t) {
  return await T(t).catch((i) => (console.error("Failed to get image meta for " + t, i + ""), { width: 0, height: 0, ratio: 0 }));
}
async function T(e) {
...
async function ee(e, t, r) {
  const i = I(e, t, { ...r });
  return typeof i.getMeta == "function" ? await i.getMeta() : await J(e, i.url);
}
function I(e, t, r) {
  var _a, _b;
...
function ie(e, t, r) {
  var _a, _b, _c, _d, _e;
  const i = y((_a = r.modifiers) == null ? void 0 : _a.width), s = y((_b = r.modifiers) == null ? void 0 : _b.height), n = Q(r.sizes), a = ((_c = r.densities) == null ? void 0 : _c.trim()) ? X(r.densities.trim()) : e.options.densities;
  Z(a);
...
const ae = K({ keyMap: { format: "f", fit: "fit", width: "w", height: "h", resize: "s", quality: "q", background: "b" }, formatter: (e, t) => q(e) + "_" + q(t.toString()) }), ce = oe({ validateDomains: true, supportsAlias: true, getImage(e, { modifiers: t, baseURL: r }, i) {
  t.width && t.height && (t.resize = `${t.width}x${t.height}`, delete t.width, delete t.height);
  const s = ae(t) || "_";
  return r || (r = b(i.options.nuxt.baseURL, "/_ipx")), { url: b(r, s, O(e).replace(/\/{2,}/g, "/")) };
...
const ue = (e) => {
  ...
  const r = e, i = t, s = A(), { providerOptions: n, normalizedAttrs: a, imageModifiers: o } = ue(r), d = p(() => s.getSizes(r.src, { ...n.value, sizes: r.sizes, densities: r.densities, modifiers: o.value })), u = U(false), m = B(), g = p(() => ({ ...a.value, "data-nuxt-img": "", ...!r.placeholder || u.value ? { sizes: d.value.sizes, srcset: d.value.srcset } : {}, ...m })), w = p(() => {
    if (u.value) return false;
    const c = r.placeholder === "" ? [10, 10] : r.placeholder;
    if (!c) return false;
    if (typeof c == "string") return c;
    const [v = 10, N = v, P = 50, j = 3] = Array.isArray(c) ? c : typeof c == "number" ? [c] : [];
    return s(r.src, { ...o.value, width: v, height: N, quality: P, blur: j }, n.value);
  }), S = p(() => r.sizes ? d.value.src : s(r.src, o.value, n.value)), l = p(() => w.value || S.value), h = W().isHydrating, f = H("imgEl");
  return C(() => {
    if (w.value || r.custom) {
      const c = new Image();
  ...
  }), (c, v) => e.custom ? V(c.$slots, "default", G(k({ key: 1 }, { imgAttrs: g.value, isLoaded: u.value, src: l.value }))) : (F(), D("img", k({ key: 0, ref_key: "imgEl", ref: f, class: w.value ? e.placeholderClass : void 0 }, g.value, { src: l.value }), null, 16, fe));
} };
export {
  ge as _
};
