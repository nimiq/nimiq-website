...
import { l as I, i as R } from "./-SGvg42Q.js";
import { x as T } from "./8Lad1M9Y.js";
import { d as k, as as O, j as C, B as D, c as S, o as X } from "./7tyaNgEA.js";
const u = 6, f = 30 * 1e3, b = 60 * 1e3, B = k({ __name: "FloatingStars.client", props: { color: { default: "#e9b213" } }, setup(w) {
  const s = O("canvas"), h = C(() => {
    var _a;
    return (_a = s.value) == null ? void 0 : _a.getContext("2d");
  }), { pixelRatio: r } = I(), d = Math.PI, x = Math.PI * 2.2, c = Array.from({ length: u * 2 }, () => d + Math.random() * (x - d)), i = D([]);
  function M() {
    if (!(!h.value || !s.value)) {
  ...
  const { resume: _ } = R(M, { immediate: false });
  T(h, () => {
    if (!s.value) return;
  ...
  return (n, e) => (X(), S("canvas", { ref_key: "canvas", ref: s, "pointer-events-none": "", "h-400": "", "w-600": "", "of-hidden": "" }, null, 512));
} }), N = Object.assign(B, { __name: "AnimatedFloatingStars" });
export {
  N as _
};
