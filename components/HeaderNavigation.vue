...
import type { NavigationDocumentData } from '~/website/prismicio-types'
import ArrowLink from './ui-kit/ArrowLink.vue'
...
<template>
  <NavigationMenu.Root v-model="currentTrigger" relative w-full flex items-center justify-end>
    <NavigationMenu.List flex="~ items-center gap-x-32 justify-between" text-neutral>
      <!-- Apps -->
        ...
        <NavigationMenu.Content class="navigation-menu" absolute left-0 top-0 min-w-max>
          <div>
            <div flex gap-20 px-20 py-32>
              ...
              <prismic-image :field="navigation.appsLinks[selectedApp]?.visual" class="relative h-full w-[300px] rounded-4 object-contain object-left-top shadow" />
            </div>
            <!-- Footer -->
      ...
      <NavigationMenu.Item v-if="navigation.getStartedGroupName && navigation.getStartedLinks.length">
        <NavigationMenu.Trigger px-16 py-4 text-16 font-semibold transition-colors nq-pill-blue focus:bg-gradient-blue-darkened class="data-[state='open']:bg-gradient-blue-darkened">
