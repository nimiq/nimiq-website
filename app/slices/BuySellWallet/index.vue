...
<template>
  <section bg-neutral-0>
    <div flex="~ col lg:row">
      ...
      <ul v-if="slice.items.length > 0" f-mt-96 grid="~ cols-1 sm:cols-2 lg:cols-1 xl:cols-2 gap-x-16 gap-y-24" mt="80 lg:0" pl="lg:16 2xl:64">
        <li v-for="({ cost, description, extraFee, isOfficial, label, logo, paymentMethod }, i) in slice.items" :key="i" shrink-0 :data-inverted="isOfficial ? '' : undefined">
          <div bg="inverted:blue neutral" p="24 xl:32" nq-no-color rounded-6 text-white>
            ...
            <PrismicRichText v-if="paymentMethod" f-mt-12 children:text-left children:text="neutral-0 inverted:white" :field="paymentMethod" />
            <PrismicRichText v-if="description" text="inverted:white/80 neutral-800" :field="description" min-h-3lh f-mt-16 />

            <p v-if="label" text="12 neutral-800 inverted:neutral-300" nq-label f-mt-32>
              {{ label }}
            </p>
          </div>
