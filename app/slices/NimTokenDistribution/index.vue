        ...
        <AccordionRoot v-model="selected" w-full>
          <AccordionItem col-span-full :value="title!">
            ...
            <AccordionContent of-hidden class="content">
              <PrismicRichText wrapper="div" :field="description" max-w-52ch />
            </AccordionContent>
          </AccordionItem>
        </AccordionRoot>
        <div v-for="j in Math.max(i - 1, 0)" :key="j" w="[calc(100%+6px)]" relative top-12.75 h-1.5 :data-state="activeIndexes.includes(Math.max(i - 1, 0) + 1) ? 'active' : undefined" transition-colors bg="neutral-500 data-active:neutral-700" />
