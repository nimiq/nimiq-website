...
<template>
  <NuxtLayout v-if="post" footer-bg-color="grey">
    <div ref="articleRef">
      <header class="max-w-$nq-prose-max-width f-pt-96/136 px-8 px-lg:64 pb-0" data-section>
        ...
        <NuxtImg v-if="post.image" class="mx-auto rounded-2 w-full object-contain f-mt-2xl" :src="post.image" :alt="post.title" view-transition-post-img />
      </header>
    </div>

    <ContentRenderer class="nq-prose" :value="post" wrapper="article" />

    <nav v-if="prev || next" class="flex flex-col flex-sm:row gap-4 flex-sm:gap-8 max-w-$nq-prose-max-width px-8 px-lg:64 f-pt-xl" data-section>
