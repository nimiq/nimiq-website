...
<template>
  <NuxtLayout v-if="post" footer-bg-color="grey">
    <div ref="articleRef">
      <header class="max-w-$nq-prose-max-width f-pt-96/136 px-32 px-lg:64 pb-0" data-section>
        ...
        <div class="mt-18 mt-lg:24 flex flex-items-center flex-gap-x-16 flex-wrap text-12 text-neutral lh-[2] nq-label md:justify-center">
          ...
          <div class="rounded-full bg-neutral-500 size-4 sm:block" hidden />
          <p class="text-neutral-800">
            {{ post.readingTime }} min
          </p>
        </div>

        <NuxtImg v-if="post.image" class="mx-auto rounded-8 w-full object-contain f-mt-2xl" :src="post.image" :alt="post.title" view-transition-post-img />
      </header>
    </div>

    <ContentRenderer class="nq-prose" :value="post" wrapper="article" />

    <nav v-if="prev || next" class="flex flex-col flex-sm:row flex-gap-16 flex-sm:gap-32 max-w-$nq-prose-max-width px-32 px-lg:64 f-pt-xl" data-section>
      <NuxtLink v-if="prev" class="flex flex-col flex-1 p-24 rounded-8 bg-neutral-100 ring-1 ring-neutral-400 nq-hoverable" :to="`/blog/${prev.slug}`">
        <span class="text-12 text-neutral-800 nq-label">Previous</span>
        <span class="text-16 text-neutral-900 mt-8 line-clamp-2">{{ prev.title }}</span>
      </NuxtLink>
      <div v-else class="flex-1" />
      <NuxtLink v-if="next" class="flex flex-col flex-1 ring-1 ring-neutral-400 p-24 text-right rounded-8 bg-neutral-100 nq-hoverable" :to="`/blog/${next.slug}`">
        <span class="text-12 text-neutral-800 nq-label">Next</span>
        <span class="text-16 text-neutral-900 mt-8 line-clamp-2">{{ next.title }}</span>
      </NuxtLink>
    </nav>
