  ...
  <NuxtLayout>
    <main>
      <section nq-section-gap bg-neutral-100>
        <div flex="~ col items-center" text-center max-w-2xl mx-auto>
          <h1 nq-heading-lg>
            {{ page.hero.title }}
          </h1>
          ...
          <form mt-48 w-full text-left @submit.prevent>
            <!-- Email Field -->
            <div mb-24>
              <label for="email" font-semibold>{{ page.form.emailLabel }}</label>
              <input id="email" v-model="email" type="email" required nq-input-box w-full mt-8>
            </div>

            <!-- Communication Permission -->
            <div mb-24>
              <p font-semibold mb-8>{{ page.form.permissionTitle }}</p>
              <p text="neutral-700 f-sm" mb-12>{{ page.form.permissionDescription }}</p>
            ...
            <!-- Interests -->
            <div mb-24>
              <p font-semibold mb-8>{{ page.form.interestsTitle }}</p>
              <p text="neutral-700 f-sm" mb-12>{{ page.form.interestsDescription }}</p>
              <div flex="~ wrap gap-x-16 gap-y-8">
                <template v-for="(interest, index) in page.form.interests" :key="interest">
                  ...
                  <span v-if="index < page.form.interests.length - 1" aria-hidden="true"> </span>
                </template>
              </div>
            </div>
            ...
            <!-- Products -->
            <div mb-24>
              <p font-semibold mb-8>{{ page.form.productsTitle }}</p>
              <div flex="~ wrap gap-x-16 gap-y-8">
                <template v-for="(product, index) in page.form.products" :key="product">
                  ...
                  <span v-if="index < page.form.products.length - 1" aria-hidden="true"> </span>
                </template>
              </div>
            </div>
