...
onMounted(() => {
  ...
  const unwatch = watch(articleEl, (el) => {
    if (el && !unwatched) {
      mediumZoom(':is(header,article) img', { margin: 24, background: 'var(--color-neutral-0)' })
      unwatched = true
      nextTick(() => unwatch())
    }
  }, { immediate: true })
...
<template>
  <NuxtLayout v-if="currentPage">
      ...
      <div class="flex flex-col w-full !max-w-[780px]">
        ...
        <div class="flex gap-x-8 gap-y-5 flex-wrap items-baseline mt-8">
          <SelectRoot v-model="selected">
            <SelectTrigger class="flex items-center gap-3 px-4 py-2.5 outline-none rounded-full bg-gradient-green" aria-label="Select Litepaper Version">
              <SelectValue class="text-xs md:text-sm text-white nq-label" />
              <Icon class="text-[10px] text-white" name="nimiq:triangle-down" />
            </SelectTrigger>

            <SelectPortal>
              <SelectContent class="rounded-[18px] bg-white min-w-[120px] w-full z-10" :side-offset="5" shadow>
                <SelectViewport class="p-1">
                  <SelectItem v-for="v in LitepaperVersion" :key="v" class="flex gap-2 items-center px-4 py-2.5 outline-none rounded-[14px] cursor-pointer transition-colors hover:bg-neutral-200 focus:bg-neutral-200" :value="v">
                    <SelectItemText class="text-sm nq-label">
                      Version {{ v }}
                    </SelectItemText>
                    <SelectItemIndicator>
                      <Icon class="text-[10px] text-neutral-800" name="nimiq:triangle-left" />
                    </SelectItemIndicator>
                  </SelectItem>
                </SelectViewport>
              </SelectContent>
    ...
    <section ref="articleEl" class="bg-neutral-100">
      <ContentRenderer v-if="selected === LitepaperVersion.V2 && posPage" class="nq-prose text-xl" style="--unit: 20px" :value="posPage" article />
      <ContentRenderer v-else-if="powPage" class="nq-prose text-xl" style="--unit: 20px" :value="powPage" article />
    </section>
