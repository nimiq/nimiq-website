...
// Enum for template usage - auto-imported globally
export const SocialMedia = {
  ...
  reddit: 'reddit',
  youtube: 'youtube',
} as const satisfies Record<SocialMediaName, SocialMediaName>
...
interface SocialMediaCollection {
  ...
  reddit: SocialMediaItem
  github: SocialMediaItem
  youtube: SocialMediaItem
  discord: SocialMediaItem
  nimiqForum: SocialMediaItem
...
export function useSocialMedias() {
  return useAsyncData('$socialMedias', async () => {
    const data = await queryCollection('socialMedia').first() as SocialMediaCollection | null

    if (!data) {
      console.warn('Social media data not found')
      return null
    }

    const socialMedias = Object.fromEntries(
      Object.entries(socialMediaConfigs).map(([platform, config]) => {
        const item = data[platform as keyof SocialMediaCollection]
        if (!item) {
          console.warn(`Platform data not found for ${platform}`)
          return [platform, { platform, link: '', label: platform, ...config }]
        }
