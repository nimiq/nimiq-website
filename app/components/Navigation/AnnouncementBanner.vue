...
<template>
  <div role="banner" p="x-32 t-20 b-24 md:y-20" aria-label="Announcement" relative z-2 w-full of-hidden bg-darkblue flex="~ col md:items-center lg:row lg:justify-center gap-y-12 gap-x-16">
    ...
    <PrismicLink v-if="hasLink(ctaLink)" internal-component="a" :field="ctaLink" z-1 mx-0 nq-pill-secondary hocus:bg-neutral-300>
      {{ ctaLabel }}
    </PrismicLink>

    <!-- Ellipses on the sides -->
    <div absolute inset-0 mx-auto max-w-3120 w-full>
      <div top="36 lg:40" right="-300 lg:-261" absolute h-159 w-606 rounded-8 op-30 blur-120 style="background: radial-gradient(141.42% 141.42% at 100% 100%, #693bc4 0%, #8f3fd5 100%);" />
    ...
    <div max-w="[calc(100%-64px)]" absolute inset-auto top-49 aspect-3.811 w-606 rounded-8 op-80 blur-120 style="background: radial-gradient(141.42% 141.42% at 100% 100%, #693bc4 0%, #8f3fd5 100%);" />
