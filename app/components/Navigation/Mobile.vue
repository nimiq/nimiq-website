            ...
            <template v-for="({ links, label }) in navigation!.blocks" :key="label">
              ...
              <AccordionItem v-if="links.length" of-hidden py-8 :value="label as string">
                ...
                <AccordionContent class="content" of-hidden>
                  <PrismicLink v-for="({ label: linkLabel, href }, j) in links" :key="j" internal-component="a" :field="href" flex="~ items-center gap-16" w-full whitespace-nowrap rounded-4 bg-white px-16 pb-10 pt-14 font-semibold>
                    {{ linkLabel }}
                  </PrismicLink>
                </AccordionContent>
              </AccordionItem>
