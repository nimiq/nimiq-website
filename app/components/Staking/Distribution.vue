...
const datum = computed(() => {
  if (!stakedSupplyRatio.value)
    return []
  return [
    { color: `rgb(var(--nq-green))`, value: stakedSupplyRatio.value || 0, label: `${formatter.format(stakedSupplyRatio.value * 100)} staked`, annotation: { bottom: '40px', right: '-72px' } },
    { color: `rgb(var(--nq-neutral-200))`, value: 1 - stakedSupplyRatio.value || 1, label: 'Circulating', annotation: { top: '40px', left: '-42px' } },
