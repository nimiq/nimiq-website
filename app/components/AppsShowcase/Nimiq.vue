...
<template>
  <ul grid="~ cols-1 lg:cols-2 gap-20 md:gap-22">
    ...
    >
      ...
      <PrismicLink internal-component="a" p="32 md:40 lg:48 b-0" :field="link" nq-hoverable of-hidden bg="neutral-300 hocus:neutral-0" style="--ring-color: transparent" grid="~ gap-x-48 lg:gap-x-80 md:rows-[auto_auto_1fr] md:cols-[221px_1fr]" :class="{ 'lg:rows-[repeat(4,auto)] lg:cols-1': !highlight }">
        <div v-if="item === 'Nimiq Pay'" size="48 lg:56" bg-gradient-gold rounded-12 grid="~ place-content-center" :class="{ 'lg:mx-auto': !highlight, 'max-xl:row-span-ful': highlight }">
          <div i-nimiq:logos-nimiq-pay-vertical-mono text="32 [#ededee]" />
        </div>
        <div v-else-if="item === 'CPL'" i-nimiq:logos-cpl-tag text-48 lg:text-56 :class="{ 'lg:mx-auto': !highlight, 'lg:ml-24': highlight }" />
        <div v-else-if="item === 'Nimiq Wallet'" i-nimiq:logos-nimiq text-56 :class="{ 'lg:mx-auto': !highlight, 'lg:ml-24': highlight }" />
        ...
        <div grid="~ place-content-center" bg-gradient-blue mb-32 rounded-full size-32 max-md:mt-12 md:hidden>
