...
<template>
  <ul grid="~ cols-1 lg:cols-2 gap-20 md:gap-22">
    ...
    >
      <PrismicLink p="32 md:40 lg:48 b-0" :field="link" of-hidden nq-hoverable bg="neutral-300 hocus:neutral-0" style="--ring-color: transparent" grid="~ gap-x-48 lg:gap-x-80 md:rows-[auto_auto_1fr] md:cols-[221px_1fr]" :class="{ 'lg:rows-[repeat(4,auto)] lg:cols-1': !highlight }">
        ...
        <div h-196 :class="{ 'max-md:mr--128 md:ml-16': item === 'CPL', 'max-md:mr--128': item === 'Nimiq Wallet', 'md:max-w-245': item === 'Nimiq Pay', 'lg:mt-40 md:row-span-full md:col-start-2 lg:row-span-1 lg:col-start-1': !highlight, 'md:row-span-full md:col-start-2 lg:h-490 lg:mb--68': highlight }" md:mx-auto>
          <ProxiedPrismicImage :field="preview" :class="{ 'max-xl:scale-155 max-xl:transform-origin-top-left': item === 'Nimiq Wallet' }" />
        </div>
      </PrismicLink>
    </li>
  </ul>
