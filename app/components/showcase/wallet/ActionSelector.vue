...
<template>
  <ul class="flex gap-1 self-end-! ring-1 ring-blue/80 p-1 rounded-full bg-white h-max relative z-10" style="--un-shadow: 0 0 32px #0682CA;">
    ...
    <li v-for="({ icon, label, value, color }) in states" :key="value" class="dark" :style="{ '--c': `var(--colors-${color})` }" scheme-dark>
      <button class="flex gap-0.5 flex-items-center p-0.5 pr-3 rounded-full bg-transparent" :data-state="value === state ? 'active' : undefined" @click="state = value">
        <Icon class="text-neutral-800 m-2 transition-color reka-active:text-$c" :name="icon" />
