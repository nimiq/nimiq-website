...
<template>
  <DefineCrypto>
    <template #default="{ adjective, crypto, fee, mainFeature, name, secondFeature, thirdFeature, time, twoColumns = false }">
      <li class="w-full max-w-[26rem] mx-auto" :class="{ 'md:grid md:rows-[max-content,auto,auto,auto] md:cols-2 xl:block gap-x-8': twoColumns }">
        <div class="flex w-full items-center justify-start gap-3 text-48 col-span-full">
          <Icon v-if="crypto === 'NIM'" name="nimiq:logos-nimiq" />
          <Icon v-else-if="crypto === 'BTC'" name="nimiq:logos-bitcoin" />
          <div v-else-if="crypto === 'USDC/USDT'" class="flex items-center justify-start gap-3">
        ...
        <div class="flex gap-2 items-center" :class="{ 'mt-3 md:mt-4': !twoColumns, 'row-start-2 col-start-2 mt-4 md:mt-6': twoColumns }">
          <div class="gradient rounded-full shrink-0 size-7 flex items-center justify-center">
  ...
  <ul v-if="items.length >= 3" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 mx-auto w-full max-w-[78rem] justify-items-center">
