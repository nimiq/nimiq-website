...
<template>
  <ul columns="1 sm:2 lg:3" style="column-rule: 2px solid var(--colors-neutral-400)" bg-neutral-0 gap-0>
    <li v-for="member in displayItems" :key="member.fullName" break-inside-avoid-column border="b-2 solid neutral-400">
      <div v-if="member.picture" px-32 pt-32>
        <NuxtImg :src="member.picture" :alt="member.fullName" rounded-6 w-full object-cover />
      </div>
      <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 20" text-neutral-400 mx-auto w-156 f-mt-xs>
      ...
      <div px-48 pb-48>
        ...
        <div v-if="member.role" f-mt-2xs flex="~ col">
          <span v-for="(r, j) in member.role.split(',').map(role => role.trim())" :key="r" font-semibold>
            {{ r }}{{ member.role.split(',').length - 1 === j ? '' : ',' }}
          </span>
        </div>
        <p nq-prose-compact f-mt-sm>
          {{ member.description }}
        </p>
        <ul v-if="member.linkedin || member.github || member.twitter" flex="~ gap-x-4" text-neutral-100 left--8 relative f-mt-xs>
          <li v-if="member.github">
            <NuxtLink :to="`https://github.com/${member.github}`" title="Github" external target="_blank" p-8 rounded-4 flex h-max bg="hocus:neutral/6" aria-label="Github link">
              <Icon name="nimiq:logos-github" class="text-20" />
            </NuxtLink>
          </li>
          <li v-if="member.twitter">
            <NuxtLink :to="`https://twitter.com/${member.twitter}`" title="Twitter" external target="_blank" p-8 rounded-4 flex h-max bg="hocus:neutral/6" aria-label="Twitter link">
              <Icon name="nimiq:logos-twitter" class="text-20" />
            </NuxtLink>
          </li>
          <li v-if="member.linkedin">
            <NuxtLink :to="`https://www.linkedin.com/in/${member.linkedin}`" title="Linkedin" external target="_blank" p-8 rounded-4 flex h-max bg="hocus:neutral/6" aria-label="Linkedin link">
              <Icon name="logos:linkedin-icon" class="text-20" />
            </NuxtLink>
          </li>
        </ul>
      </div>
