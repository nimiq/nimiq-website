...
<template>
  <ul class="bg-neutral-0 gap-0" columns="1 sm:2 lg:3" style="column-rule: 2px solid var(--colors-neutral-400)">
    <li v-for="member in displayItems" :key="member.fullName" class="break-inside-avoid-column border-b-2 border-solid border-neutral-400">
      <div v-if="member.picture" class="px-8 pt-8">
        <NuxtImg class="rounded-1.5 w-full object-cover" :src="member.picture" :alt="member.fullName" />
      </div>
      <svg v-else class="text-neutral-400 mx-auto w-[156px] mt-2 md:mt-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 20">
      ...
      <div class="px-12 pb-12">
        ...
        <div v-if="member.role" class="mt-1 md:mt-1.5 flex flex-col">
          <span v-for="(r, j) in member.role.split(',').map(role => role.trim())" :key="r" class="font-semibold">
            {{ r }}{{ member.role.split(',').length - 1 === j ? '' : ',' }}
          </span>
        </div>
        <p class="nq-prose-compact mt-3 md:mt-4">
          {{ member.description }}
        </p>
        <ul v-if="member.linkedin || member.github || member.twitter" class="flex gap-x-1 text-neutral-100 -left-2 relative mt-2 md:mt-3">
          <li v-if="member.github">
            <NuxtLink class="p-2 rounded-1 flex h-max bg-hocus:neutral/6" :to="`https://github.com/${member.github}`" title="Github" external target="_blank" aria-label="Github link">
              <Icon class="text-20" name="nimiq:logos-github" />
            </NuxtLink>
          </li>
          <li v-if="member.twitter">
            <NuxtLink class="p-2 rounded-1 flex h-max bg-hocus:neutral/6" :to="`https://twitter.com/${member.twitter}`" title="Twitter" external target="_blank" aria-label="Twitter link">
              <Icon class="text-20" name="nimiq:logos-twitter" />
            </NuxtLink>
          </li>
          <li v-if="member.linkedin">
            <NuxtLink class="p-2 rounded-1 flex h-max bg-hocus:neutral/6" :to="`https://www.linkedin.com/in/${member.linkedin}`" title="Linkedin" external target="_blank" aria-label="Linkedin link">
