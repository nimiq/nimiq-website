...
<template>
  <div :data-state="showAllValidators ? 'open' : 'closed'" :class="showAllValidators ? 'h-auto' : 'h-448 of-y-clip'" :style="`--count: ${validators?.length}`" pb-80 w-full relative transition="height discrete">
    ...
    <button bottom="8 data-open:42" nq-pill-secondary mx-auto transition-bottom inset-x-0 absolute z-10 aria-label="Expand list" @click="toggleShowAllValidators">
    ...
    <AccordionRoot v-model="activeValidator" type="single" :collapsible="true" grid="~ cols-[repeat(auto-fit,minmax(200px,469px))] gap-16 justify-center" as="ul" w-full transition-height>
      <AccordionItem v-for="({ name, address, logo, description, rewardPerAnnum, score, fee, website, dominanceRatio }) in validators" :key="name" as="li" :value="address" style="--reka-accordion-content-height: 130px" rounded-8 max-w-full transition relative>
        <AccordionTrigger rounded="8 data-open:b-0" data-open:border="1 neutral-400 b-transparent" p="x-20 lg:x-24 16" grid="~ cols-[max-content_1fr_max-content] rows-[1fr_max-content] gap-x-16 items-center" :aria-label="`See more details about ${name}`" border="1 transparent" transition="[background-color,border-radius,border]" rounded-8 size-full delay-100 relative data-open:z-11 bg="neutral-200 data-open:neutral-0" @click="handleTriggerClick">
          ...
          <p v-if="rewardPerAnnum" text="f-sm neutral-800 left" col="start-2 span-2" font-normal mt-0 row-start-2>
            {{ percentageFormatter.format(rewardPerAnnum) }} p.a.
          </p>
        </AccordionTrigger>
        <AccordionContent class="animate-content" p="x-24 t-0 b-24" border="1 neutral-400 t-0" top-full pt-8 rounded-b-8 bg-neutral-0 shadow transition-padding inset-x-0 absolute z-10 z-11 of-y-clip>
          ...
          <template v-if="description || website">
            <hr w="[calc(100%+48px)]" mx--24 bg-neutral-300 h-1 f-mt-sm>
            <p v-if="description" text="neutral" font-500>
              &ldquo;{{ description }}&rdquo;
            </p>
            <NuxtLink v-if="website" :to="website" un-text="blue" external nq-arrow font-semibold mt-8>
              {{ website }}
            </NuxtLink>
