...
const scoreAccordionExpanded = ref<Record<string, boolean>>({})
...
function onScoreAccordionExpandedChange(address: string, value: boolean) {
  scoreAccordionExpanded.value[address] = value
}
</script>

<template>
  <div class="w-full">
    <div :data-state="showAllValidators ? 'open' : 'closed'" class="w-full relative transition-[height]" :class="[showAllValidators ? 'h-auto' : 'h-[448px] md:h-[320px]', !showAllValidators && !activeValidator && 'overflow-y-clip']" :style="`--count: ${validators?.length}`">
      <div class="h-[192px] w-full pointer-events-none bottom-0 absolute z-10 transition-opacity" :class="showAllValidators ? 'opacity-0' : 'opacity-100'" aria-hidden style="background: linear-gradient(to bottom in oklab, transparent 0%, rgb(255, 255, 255) 100%);" />
      <AccordionRoot v-model="activeValidator" type="single" :collapsible="true" as="ul" class="grid grid-cols-1 md:grid-cols-2 gap-4 mx-auto w-full transition-[height]">
        <AccordionItem v-for="({ name, address, logo, description, rewardGainRatio, score, fee, website, dominanceRatio }) in validators" :key="name" as="li" :value="address" class="rounded-2 w-full max-w-full relative">
          <AccordionTrigger class="validator-trigger rounded-2 border border-transparent transition-[background-color,border-color,border-radius] duration-250 ease-out delay-75 relative reka-open:z-[11] bg-neutral-200 p-6 flex items-center gap-4 w-full" :aria-label="`See more details about ${name}`">
          ...
          <AccordionContent
            class="accordion-content px-6 pb-8 border border-neutral-400 border-t-0 rounded-b-2 bg-neutral-0 shadow inset-x-0 top-full absolute z-[11]"
            :class="scoreAccordionExpanded[address] ? 'overflow-y-auto max-h-[70vh]' : 'overflow-y-visible max-h-none'"
          >
            <div class="bg-neutral-0 h-1 inset-x-0 top-0 absolute z-[111] sticky" />
            <ValidatorScoreAccordion
              v-if="score"
              :score
              class="mt-4"
              @expanded-change="onScoreAccordionExpandedChange(address, $event)"
            />
            <div class="grid grid-cols-[1fr_auto] items-center text-neutral-700 text-sm font-semibold mt-2">
              <template v-if="rewardGainRatio !== undefined">
                <h4 class="text-[16px] text-neutral">
            ...
            <template v-if="description || website">
              <hr class="w-[calc(100%+48px)] mx-[-24px] h-[1px] my-4 border-0 border-t border-current text-neutral-300">
              ...
              <p class="text-neutral-700 text-[10px] mt-1">
            ...
            <hr class="w-[calc(100%+48px)] mx-[-24px] h-[1px] mt-4 mb-3 border-0 border-t border-current text-neutral-300">
            <button class="copy-address group flex items-center gap-2 w-full rounded-1 px-3 py-2 mb-1 transition-colors" :class="copiedAddress === address ? 'bg-green/8' : 'hover:bg-neutral-200'" @click.stop="copyAddress(address)">
              <span class="font-mono text-[13px] leading-none transition-colors flex-1 min-w-0 text-left md:whitespace-nowrap" :class="copiedAddress === address ? 'text-green' : 'text-neutral-700'">{{ address }}</span>
              <span class="relative size-4 ml-auto shrink-0">
                <Transition enter-active-class="transition-[opacity,transform] duration-200 ease-out" enter-from-class="opacity-0 scale-75" enter-to-class="opacity-100 scale-100" leave-active-class="transition-[opacity,transform] duration-150 ease-out" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-75" mode="out-in">
                  <Icon v-if="copiedAddress === address" class="size-3 text-green absolute inset-0" name="nimiq:check" />
                  <Icon v-else class="size-3 text-neutral-600 group-hover:text-neutral-800 transition-colors absolute inset-0" name="lucide:copy" />
                </Transition>
              </span>
            </button>
          </AccordionContent>
    ...
    <button class="mx-auto mt-6 md:mt-8 relative z-10 nq-pill-secondary" aria-label="Expand list" @click="toggleShowAllValidators">
...
:deep(.validator-trigger[data-state='open']) {
  animation: validator-trigger-open 240ms var(--ease-out-quart) both;
...
:deep(.validator-trigger[data-state='closed']) {
  animation: validator-trigger-close 180ms var(--ease-out-quart) both;
}

@keyframes validator-trigger-open {
  from {
...
@keyframes validator-trigger-close {
  from {
    background-color: var(--color-neutral-0);
    border-color: var(--color-neutral-400);
    border-bottom-color: transparent;
