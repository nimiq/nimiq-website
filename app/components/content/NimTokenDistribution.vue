...
interface NimTokenDistributionSlice {
  primary: {
    item: any[]
  }
}

const { slice } = defineProps<{ slice: NimTokenDistributionSlice, slices?: any[] }>()

const gridTemplateColumns = computed(() => slice.primary.item.map((i: any) => `${i.percentage}fr`).join(' '))

const selected = ref<string[]>([])

const activeIndexes = computed(() => slice.primary.item.filter((i: any) => selected.value.includes(i.title!)).map((i: any) => slice.primary.item.map((i: any) => i.title!).indexOf(i.title!)))
</script>
