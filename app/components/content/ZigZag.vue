...
<template>
  <ul>
    <li v-for="(item, i) in items" :key="i" class="grid grid-cols-1 gap-x-96 md:grid-cols-2 md:items-center border-b border-neutral-300 last:border-b-0" :class="{ 'md:pr-32': i % 2 !== 0, 'md:pl-32': i % 2 === 0 }">
      <NuxtImg v-if="item.image" class="ring-1.5 ring-neutral-400 aspect-[1.39] md:aspect-[1.48] lg:aspect-auto rounded-6 pointer-events-none object-cover shadow" :src="item.image" :alt="item.headline" :class="{ 'md:col-start-2': i % 2 === 0 }" />
