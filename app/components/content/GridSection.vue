...
const props = defineProps<{ slice: GridSectionSlice, slices?: any[] }>()
  ...
  <section :class="bgClass" data-slice-type="grid-section">
    <template v-if="slice.variation === 'default'">
      <ul nq-grid-330>
        ...
        >
          <ProxiedPrismicImage :field="item.icon" size="48 lg:56 xl:64" />
    ...
    <template v-else-if="slice.variation === 'threeColumnsImageText'">
      <ul flex="~ col lg:row gap-x-24 gap-y-40 items-center" w-full>
        <li v-for="({ headline, image }, i) in slice.items" :key="i" flex-1 w-full>
          <ProxiedPrismicImage :field="image" w="full md:w-max lg:full" rounded flex max-h-229 object="contain md:cover lg:contain" lg:mx-auto dark:bg-neutral-200 />
          <PrismicText wrapper="h3" text="20/26 md:center" mt-24 min-h-2lh lg:mx-auto lg:max-w-24ch :field="headline" />
        </li>
      </ul>
    </template>
    <template v-else-if="slice.variation === 'threeColumnsIconsText'">
      <ul flex="~ col lg:row gap-y-24 lg:items-center" w-full>
        <li v-for="({ headline }, i) in slice.items" :key="i" py-24 flex-1 border="b-1 lg:b-0 lg:r-1 last:0 white/15 solid" md:mx-auto md:w-max>
    ...
    <template v-else-if="slice.variation === 'threeColumnsIconsInline'">
      <ul flex="~ col lg:row gap-y-24 md:items-center" w-full>
        <li v-for="({ content, icon }, i) in slice.primary.items" :key="i" flex-1 flex="~ gap-x-12 items-center">
