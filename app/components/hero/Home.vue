...
<template>
  <div class="flex-grow flex flex-col flex-justify-center z-10" children:md:mx-auto>
    ...
    <p v-if="description" class="text-neutral-800 text-f-xl">
      {{ description }}
    </p>
    ...
    <div v-if="organizations?.length" class="mt-80 flex flex-col flex-md:row flex-wrap gap-32 justify-start justify-md:center items-start items-md:center">
      <span class="text-neutral-700 nq-label">Works with</span>
      <div class="flex flex-wrap gap-32 justify-start justify-md:center items-center">
        <NuxtLink v-for="(item, i) in organizations" :key="i" class="op-85 op-hocus:100 transition-opacity" :to="item.url" :class="item.url?.startsWith('https://naka.com') ? 'h-26' : 'h-32'" target="_blank">
  ...
  <UiEmber class="left-100 top-77 top-sm:112 top-md:200" style="--scale: 1.05" />
  <UiEmber class="right-80 top-170" style="--scale: 0.7" />
  <UiEmber class="bottom-302 bottom-md:430 left-56vw left-md:30vw" style="--scale: 0.92" />
  <UiEmber class="bottom-460 right-32" style="--scale: 1" />

  <!-- Map -->
  <div class="mt-120 max-w-none w-full flex flex-justify-center">
