...
<template>
  <section class="bg-neutral-0 relative of-x-clip f-p-5xl max-md:pb-100">
    <!-- Background layers for the green radial gradient effect -->
    <AnimatedCircleRipple class="left--100vw left-lg:-5vw size-300vw size-lg:110vw max-w-none absolute z-0 bottom-10 bottom-lg:-300" />
    <div class="h-400 h-lg:200 bg-neutral-0 max-w-none pointer-events-none inset-0 absolute z-1" />
    ...
    <div class="flex flex-col flex-md:items-center group relative z-2">
      <AnimatedStakingRipple class="size-120" />
      <h1 class="nq-heading text-wrap text-md:balance f-mt-md md:text-center">
        {{ headlineParts[0] }}
        <span class="bg-green/15 inline-flex-~ text-green px-10 py-3 rounded-4 whitespace-nowrap" data-percentage>
      ...
      <small :id class="text-green-1100 text-sm:center transition-colors duration-400 f-mt-md sm:max-w-40ch" data-note>
        <Icon class="size-0.4em translate-x--8 translate-y-4 sm:translate-x-4 sm:translate-y-8" aria-hidden name="nimiq:asterisk" /> average return based on the current distribution. This is not financial advice.
      </small>
    </div>
  </section>
</template>
