...
<template>
  <div class="py-80 py-md:120 flex flex-col flex-items-center px-0 text-center bg-neutral-0 relative of-x-clip">
    <h1 class="nq-heading-lg max-w-prose">
      {{ data.title }}
    </h1>
    <p v-if="data.description" class="text-neutral-700 text-f-lg mt-24 max-w-prose">
      {{ data.description }}
    </p>

    <!-- Price chart ribbon -->
    <div class="mt-64 w-full">
      <UiRibbonContainer v-if="data.nimPriceChartLabel" class="z-3 md:min-h-480 outline-color-white/20" :label="data.nimPriceChartLabel" shadow>
        <div class="grid grid-cols-1 grid-md:cols-[max-content_1fr] size-full relative of-hidden">
          <aside ref="asideRef" class="md:border-r-1 md:border-solid md:border-neutral-400 grid grid-cols-[repeat(4,1fr)] grid-md:cols-1 grid-gap-col-20 grid-gap-row-24 w-full relative f-p-md max-md:row-start-2 max-md:of-x-auto">
            <transition enter-active-class="transition duration-200 ease-out" enter-from-class="op-0" enter-to-class="op-100" leave-active-class="transition duration-200 ease-out" leave-from-class="op-100" leave-to-class="op-0">
              <div v-if="isLoading || error" class="flex flex-items-center flex-gap-8 text-orange text-f-sm translate-x-100% py-4 rounded-br-6 bg-white right--1 top--1 absolute z-30 f-px-xs border-b border-r border-neutral-400 lg:w-max">
                <Icon class="scale-90" :name="isLoading ? 'nimiq:spinner' : 'nimiq:alert'" />
                <p>{{ isLoading ? 'Loading...' : error }}</p>
              </div>
            </transition>

            <div v-if="data.marketCapLabel" class="flex flex-col flex-gap-8 relative z-1 reka-inactive:animate-pulse" :data-state="marketCapFormatted === '0' ? 'inactive' : 'active'">
              <div class="outline outline-1.5 outline-neutral-200 outline-offset--1.5 rounded-6 bg-neutral-100 h-full w-full inset--12 absolute z--1 reka-active:hidden w-[calc(100%+24px)]" />
              <div class="flex flex-gap-8 flex-items-center">
                <span class="text-f-lg text-neutral lh-none font-semibold whitespace-nowrap">{{ marketCapFormatted }}</span>
                <div v-if="marketCapChange" class="flex flex-gap-2 flex-items-center" :class="marketCapChange < 0 ? 'text-red' : 'text-green'">
                  <Icon class="size-7" name="nimiq:triangle-up" aria-hidden :class="{ 'rotate-180': marketCapChange < 0 }" />
            ...
            <div v-if="data.volume24HLabel" class="flex flex-col flex-gap-8 relative z-1 reka-inactive:animate-pulse" :data-state="volumeFormatted === '0' ? 'inactive' : 'active'">
              <div class="outline outline-1.5 outline-neutral-200 outline-offset--1.5 rounded-6 bg-neutral-100 h-full w-full inset--12 absolute z--1 reka-active:hidden w-[calc(100%+24px)]" />
              <div class="flex flex-gap-8 flex-items-center">
                <span class="text-f-lg text-neutral lh-none font-semibold whitespace-nowrap">{{ volumeFormatted }}</span>
                <div v-if="volumeChange" class="flex flex-gap-2 flex-items-center" :class="volumeChange < 0 ? 'text-red' : 'text-green'">
                  <Icon class="size-7" name="nimiq:triangle-up" aria-hidden :class="{ 'rotate-180': volumeChange < 0 }" />
            ...
            <div v-if="data.totalSupplyLabel" class="flex flex-col flex-gap-8 relative z-1 reka-inactive:animate-pulse" :data-state="currentSupplyFormatted === '0' ? 'inactive' : 'active'">
              <div class="outline outline-1.5 outline-neutral-200 outline-offset--1.5 rounded-6 bg-neutral-100 h-full w-full inset--12 absolute z--1 reka-active:hidden w-[calc(100%+24px)]" />
            ...
            <div v-if="data.maxSupplyLabel" class="flex flex-col flex-gap-8 relative z-1 reka-inactive:animate-pulse" :data-state="maxSupplyFormatted === '0' ? 'inactive' : 'active'">
              <div class="outline outline-1.5 outline-neutral-200 outline-offset--1.5 rounded-6 bg-neutral-100 h-full w-full inset--12 absolute z--1 reka-active:hidden w-[calc(100%+24px)]" />
          ...
          <div class="group relative f-pb-xs">
            <ChartLine :key="asideWidth" class="rounded-8 h-full" :data="historicPrices || []" leader>
              <template #default="{ data: [ts, historicPrice] }">
                <div class="flex flex-col flex-gap-8 f-$side f-$side-min-20 f-$side-max-24 mx-20 bg-neutral-0 top-21 relative f-py-xs">
            ...
            <div class="right-32 top-32 absolute z-20">
              <div v-if="historicPrices?.at(-1)" class="flex flex-col flex-gap-8 f-$side f-$side-min-20 f-$side-max-24 bg-neutral-0 transition-opacity relative f-py-xs leader-hocus:invisible">
                <div class="inset-y-0 absolute left-[calc(var(--f-side)*-1)] w-$f-side" style="background-image: linear-gradient(to right in oklab, transparent, var(--colors-neutral-0))" />
                <div class="right-[calc(var(--f-side)*-1)] w-$f-side inset-y-0 absolute f-w-md" style="background-image: linear-gradient(to right in oklab, var(--colors-neutral-0), transparent)" />
                <p class="text-blue text-f-3xl lh-none font-semibold">
                ...
                <div v-if="deltaPrice && currencyInfo" class="flex flex-items-center text-f-2xs text-neutral-700 lh-none font-semibold">
                  <Icon class="mr-4 size-8" name="nimiq:triangle-up" :class="{ 'rotate-180': deltaPrice < 0 }" />
                  <span>{{ formatDecimal(Math.abs(deltaPrice), locale.value) }} ({{ formatPercentage(deltaPrice / historicPrices.at(-1)![1], locale.value) }})</span>
                </div>
              </div>
            </div>

            <div class="flex flex-items-center flex-gap-8 absolute z-20 f-right-md" data-allow-mismatch :class="controlsPosition === 'top' ? 'top-128' : 'bottom-64'">
              <UiPillSelector v-model="period" class="self-end justify-self-end ring-white ring-3" :options="periodOptions" />
              <UiCurrencySelector v-model="currency" class="bg-darkblue bg-hocus:neutral-200 transition-[background-color] text-14 font-normal px-2 rounded-full h-full nq-label text-white text-hocus:neutral ring-white ring-3 border border-1.5 border-neutral-200">
    ...
    <p v-if="data.poweredByLabel && data.poweredByLogo" class="flex flex-items-center flex-justify-center flex-gap-8 f-mt-md text-center text-f-sm">
      {{ data.poweredByLabel }}
