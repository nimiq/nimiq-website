...
<template>
  <div class="py-20 py-md:120 flex flex-col flex-items-center px-0 text-center bg-neutral-0 relative of-x-clip">
    ...
    <div class="mt-16 w-full">
      <UiRibbonContainer v-if="data.nimPriceChartLabel" class="z-3 md:min-h-[480px] outline-color-white/20" :label="data.nimPriceChartLabel" shadow>
        <div class="grid grid-cols-1 grid-md:cols-[max-content_1fr] size-full relative of-hidden">
          <aside ref="asideRef" class="md:border-r-1 md:border-solid md:border-neutral-400 grid grid-cols-[repeat(4,1fr)] grid-md:cols-1 grid-gap-col-20 grid-gap-row-24 w-full relative p-4 md:p-6 max-md:row-start-2 max-md:of-x-auto">
            <transition enter-active-class="transition duration-200 ease-out" enter-from-class="op-0" enter-to-class="op-100" leave-active-class="transition duration-200 ease-out" leave-from-class="op-100" leave-to-class="op-0">
              <div v-if="isLoading || error" class="flex flex-items-center gap-2 text-orange text-xs md:text-sm translate-x-100% py-1 rounded-br-1.5 bg-white -right-px -top-px absolute z-30 px-2 md:px-3 border-b border-r border-neutral-400 lg:w-max">
            ...
            <div v-if="data.marketCapLabel" class="flex flex-col gap-2 relative z-1 reka-inactive:animate-pulse" :data-state="marketCapFormatted === '0' ? 'inactive' : 'active'">
              ...
              <div class="flex gap-2 flex-items-center">
                <span class="text-base md:text-lg text-neutral lh-none font-semibold whitespace-nowrap">{{ marketCapFormatted }}</span>
                <div v-if="marketCapChange" class="flex gap-0.5 flex-items-center" :class="marketCapChange < 0 ? 'text-red' : 'text-green'">
                  ...
                  <span class="lh-none font-semibold text-xs md:text-sm">{{ formatPercentage(marketCapChange, locale) }}</span>
                </div>
              </div>
              <p class="text-[11px] md:text-xs text-neutral-800 lh-none font-normal">
                {{ data.marketCapLabel }}
              </p>
            </div>
            ...
            <div v-if="data.volume24HLabel" class="flex flex-col gap-2 relative z-1 reka-inactive:animate-pulse" :data-state="volumeFormatted === '0' ? 'inactive' : 'active'">
              <div class="outline outline-1.5 outline-neutral-200 outline-offset--1.5 rounded-1.5 bg-neutral-100 h-full w-full -inset-3 absolute z--1 reka-active:hidden w-[calc(100%+24px)]" />
              <div class="flex gap-2 flex-items-center">
                <span class="text-base md:text-lg text-neutral lh-none font-semibold whitespace-nowrap">{{ volumeFormatted }}</span>
                <div v-if="volumeChange" class="flex gap-0.5 flex-items-center" :class="volumeChange < 0 ? 'text-red' : 'text-green'">
                  ...
                  <span class="lh-none font-semibold text-xs md:text-sm">{{ formatPercentage(volumeChange, locale) }}</span>
                </div>
              </div>
              <p class="text-[11px] md:text-xs text-neutral-800 lh-none font-normal">
                {{ data.volume24HLabel }}
            ...
            <div v-if="data.totalSupplyLabel" class="flex flex-col gap-2 relative z-1 reka-inactive:animate-pulse" :data-state="currentSupplyFormatted === '0' ? 'inactive' : 'active'">
              <div class="outline outline-1.5 outline-neutral-200 outline-offset--1.5 rounded-1.5 bg-neutral-100 h-full w-full -inset-3 absolute z--1 reka-active:hidden w-[calc(100%+24px)]" />
              <span class="text-base md:text-lg text-neutral lh-none font-semibold">{{ currentSupplyFormatted }}</span>
              <p class="text-[11px] md:text-xs text-neutral-800 lh-none font-normal">
            ...
            <div v-if="data.maxSupplyLabel" class="flex flex-col gap-2 relative z-1 reka-inactive:animate-pulse" :data-state="maxSupplyFormatted === '0' ? 'inactive' : 'active'">
              <div class="outline outline-1.5 outline-neutral-200 outline-offset--1.5 rounded-1.5 bg-neutral-100 h-full w-full -inset-3 absolute z--1 reka-active:hidden w-[calc(100%+24px)]" />
              <span class="text-base md:text-lg text-neutral lh-none font-semibold">{{ maxSupplyFormatted }}</span>
              <p class="text-[11px] md:text-xs text-neutral-800 lh-none font-normal">
                {{ data.maxSupplyLabel }}
              </p>
            </div>

            <div v-show="lastUpdated" class="flex flex-md:col flex-gap-col-4 flex-gap-row-8 flex-max-md:justify-center text-[10px] md:text-[11px] text-neutral-800 lh-none md:mt-auto" max-md="col-span-full sticky left-0 w-[calc(100vw-80px)] w-max">
          ...
          <div class="group relative pb-2 md:pb-3">
            <ChartLine :key="asideWidth" class="rounded-2 h-full" :data="historicPrices || []" leader>
              <template #default="{ data: [ts, historicPrice] }">
                <div class="flex flex-col gap-2 f-$side f-$side-min-20 f-$side-max-24 mx-5 bg-neutral-0 top-[21px] relative py-2 md:py-3">
                  ...
                  <div class="right-[calc(var(--f-side)*-1)] w-$f-side inset-y-0 absolute w-4 md:w-6" style="background-image: linear-gradient(to right in oklab, var(--colors-neutral-0), transparent)" />
                  <p class="text-blue text-2xl md:text-3xl lh-none font-semibold">
                    {{ currencyInfo ? formatFiat(historicPrice, currencyInfo, locale) : '' }}
                  </p>
                  <NuxtTime class="text-[10px] md:text-[11px] text-right text-neutral-700 lh-none nq-label" :datetime="ts" year="numeric" month="long" day="numeric" />
                </div>
              </template>
            </ChartLine>
            ...
            <div class="right-8 top-8 absolute z-20">
              <div v-if="historicPrices?.at(-1)" class="flex flex-col gap-2 f-$side f-$side-min-20 f-$side-max-24 bg-neutral-0 transition-opacity relative py-2 md:py-3 leader-hocus:invisible">
                ...
                <div v-if="deltaPrice && currencyInfo" class="flex flex-items-center text-[10px] md:text-[11px] text-neutral-700 lh-none font-semibold">
            ...
            <div class="flex flex-items-center gap-2 absolute z-20 right-4 md:right-6" data-allow-mismatch :class="controlsPosition === 'top' ? 'top-32' : 'bottom-16'">
    ...
    <p v-if="data.poweredByLabel && data.poweredByLogo" class="flex flex-items-center flex-justify-center gap-2 mt-4 md:mt-6 text-center text-xs md:text-sm">
      {{ data.poweredByLabel }}
      <NuxtLink v-if="data.poweredByLink" :to="data.poweredByLink" external target="_blank">
        <NuxtImg class="op-80 op-hocus:100 w-full transition-opacity h-3 md:h-4" :src="data.poweredByLogo" />
      </NuxtLink>
      <NuxtImg v-else class="op-80 w-full h-3 md:h-4" :src="data.poweredByLogo" />
    </p>
  </div>
</template>
