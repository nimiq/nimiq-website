...
<template>
  <div class="grid gap-x-1.5 grid-flow-row gap-y-5 bg-neutral-0 w-full" :style="{ gridTemplateColumns, gridTemplateRows: `repeat(${items.length + 1}, auto)` }">
    ...
    <template v-for="({ title, description, percentage }, i) in items" :key="i">
        ...
        <AccordionItem class="col-span-full" :value="title">
          <AccordionHeader>
            <AccordionTrigger class="flex items-center gap-x-2 bg-transparent w-full">
              <span class="font-semibold transition-colors text-base md:text-lg" :style="{ color: selected.length === 0 || selected.includes(title) ? `var(--color-${getColor(i)})` : `var(--color-neutral-700)` }">
                {{ title }}
          ...
          <AccordionContent class="overflow-hidden animate-accordion">
            <p class="max-w-52ch">
              {{ description }}
            </p>
          </AccordionContent>
      ...
      <div v-if="i > 0" class="relative overflow-hidden" :data-state="activeIndexes.includes(Math.max(i - 1, 0) + 1) ? 'active' : undefined">
