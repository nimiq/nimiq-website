...
const props = defineProps<{ items: TokenItem[] }>()

const gridTemplateColumns = computed(() => props.items.map(i => `${i.percentage}fr`).join(' '))

const selected = ref<string[]>([])

const activeIndexes = computed(() => props.items.filter(i => selected.value.includes(i.title)).map(i => props.items.map(item => item.title).indexOf(i.title)))
</script>

<template>
  <section bg-neutral-0>
    <div grid="~ gap-x-6 flow-row gap-y-20" :style="{ gridTemplateColumns, gridTemplateRows: `repeat(${items.length + 1}, auto)` }" w-full>
      <div v-for="({ color }, i) in items" :key="i" :style="`background-image:var(--nq-${color}-gradient)`" rounded-6 h-81 shadow />
      <template v-for="({ title, description, color, percentage }, i) in items" :key="i">
        ...
        <div v-for="j in items.length - i - 1" :key="j" h="[calc(100%+40px)]" :data-state="activeIndexes.includes(j + i) ? 'active' : undefined" w-1.5 self-center justify-self-center bg="neutral-500 data-active:neutral-700" />
      </template>
    </div>
  </section>
</template>
