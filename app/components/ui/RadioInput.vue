...
<template>
  <div :id class="flex gap-0.5 items-center group rounded-1 bg-neutral-200 w-max relative">
    ...
    <label v-for="(option, i) in options" :key="i" class="z-1" :data-state="isSelected(option) ? 'active' : undefined">
      ...
      <div class="text-12 leading-none tracking-0.8 px-2 py-1 cursor-pointer transition-colors relative nq-label" :class="isSelected(option) ? 'text-neutral-0' : 'text-neutral-800'">
