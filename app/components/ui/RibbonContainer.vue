...
const { floatingStars = true, color = 'gold', icon, shadow = false, corner = false } = defineProps<{ label: string, floatingStars?: boolean, color?: 'gold' | 'purple', icon?: string, shadow?: boolean, corner?: boolean }>()
...
<template>
  <div class="ribbon-container max-w-[calc(100vw-16px)] md:max-w-[var(--nq-max-width)] mx-auto rounded-lg size-full min-h-32 relative grid outline outline-[1.5px] outline-neutral-400 bg-neutral-0" :class="{ shadow }" style="--top: -25px; --right: -32px">
    ...
    <div class="self-start justify-self-end w-max relative z-10" :style="{ top: corner ? '0px' : 'var(--top, -25px)', marginRight: corner ? '0px' : undefined }" :class="!corner && 'md:mr-[var(--right,-32px)]'">
      ...
      <LazyUiFloatingStars v-if="floatingStars" class="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 top-[30%] z-[21]" aria-hidden="true" :color="starColorMap[color]" />
