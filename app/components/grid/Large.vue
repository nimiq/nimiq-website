...
interface LargeGridItem { shape?: string, headline: string, subline?: string, icon?: string, iconName?: string, linkHref?: string, color?: string }
defineProps<{ items: LargeGridItem[] }>()
</script>

<template>
  <div grid="~ gap-20 cols-[repeat(auto-fit,minmax(min(100%,400px),1fr))] justify-center" w-full>
    <NuxtLink v-for="(item, i) in items" :key="i" :style="`--c:${item.color}`" :to="item.linkHref" p="32 lg:40 2xl:48" group relative nq-hoverable class="hocus:var:nq-gradient-from:$c hocus:var:nq-gradient-to:$c">
      ...
      <Icon v-if="item.iconName" :name="item.iconName" class="nq-prose-compact text-96 text-blue max-w-128 f-mb-sm md:text-120 group-hocus:text-white" :class="{ 'md:mx-auto': item.subline }" />
