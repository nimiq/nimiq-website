...
<template>
  ...
  >
    <div flex="grow ~ col justify-center" z-10 children:md:mx-auto>
      ...
      <p v-if="props.subheadline" text="neutral-800 f-xl">
        {{ props.subheadline }}
      </p>
      <NuxtLink v-if="props.link" :to="props.link" mt-40 nq-arrow nq-pill-lg nq-pill-blue>
        Go to Wallet
      </NuxtLink>
      <div v-if="props.organizations?.length" mt-80 flex="~ col md:row wrap" gap-32 justify="start md:center" items="start md:center">
        <span text-neutral-700 nq-label>Works with</span>
        <div flex="~ wrap" gap-32 justify="start md:center" items-center>
          <NuxtLink v-for="(item, i) in props.organizations" :key="i" :to="item.url" :class="item.url?.startsWith('https://naka.com') ? 'h-26' : 'h-32'" op="85 hocus:100" transition-opacity target="_blank">
