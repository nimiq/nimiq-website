...
const items = computed(() => {
  switch (props.background) {
    ...
    case 'Nimiq Apps': {
      ...
      if (apps.length < 4)
        return []
      return apps.sort(() => Math.random() - 0.5).slice(0, 4).map(({ color, logo, linkHref, name }, i) => ({ color, icon: logo, link: linkHref, classes: `text-52 ${classesPositions[i]}`, name: name! }))
    }
    default : return []
  }
})
...
<template>
  <div border="1 solid neutral-400" relative w-full of-hidden rounded-8 bg-neutral-0 px-32 py-72 nq-shadow>
    <BannerSocialMediaBackground v-if="items.length > 0" :items />
